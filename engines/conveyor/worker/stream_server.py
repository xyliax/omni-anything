    ...
    async def _run_session(self, sid: int, st: Session):
        ...
        async def gen():
            ...
            if self.initial_context_tokens:
                ...
                filler = " ".join(_r.choice(_vocab) for _ in range(self.initial_context_tokens))
                tokenizer = self.engine.get_tokenizer()
                prompt_ids = tokenizer.encode(self.input_adapter.preload(f"[context {sid}] " + filler))[:self.initial_context_tokens]
                if len(prompt_ids) != self.initial_context_tokens:
                    raise ValueError('initial context shorter than requested token count')
        ...
        try:
            ...
            async for out in self.engine.generate(gen(), base_sp, f"s{sid}e1"):
                ...
                if self.initial_context_tokens and st.tokens and st.consumed == 0:
                    # The state-construction token is never service output,
                    # including histories initialized after session arrival.
                    st.consumed = 1
                    st.consumed_text = len(self.engine.get_tokenizer().decode(
                        st.tokens[:1], skip_special_tokens=True))
...
def main():
    ...
    if args.initial_context_tokens and args.preload_sessions:
        ...
        t0 = time.monotonic()
        managed_preload = bool(os.environ.get('OMNI_SESSION_MANAGER') and os.environ.get('OMNI_ADMISSION_PROFILE'))
        def utility(name, *values):
            return asyncio.run_coroutine_threadsafe(
                eng.engine.engine_core.call_utility_async(name, *values), eng.loop).result(timeout=60)
        if managed_preload:
            import json
            profile = json.loads(Path(os.environ['OMNI_ADMISSION_PROFILE']).read_text())
            eviction_budget = int(profile['max_evict_blocks'])
            utility('initial_context_finalize')
        for sid in range(1, n + 1):
            if managed_preload:
                utility('session_plan', f's{sid}e1', 2.0, time.time() + 3600, 0.35, 1, None, eviction_budget)
            eng._ensure(sid)
            deadline = time.monotonic() + 90
            while not eng.sessions[sid].tokens:
                if time.monotonic() > deadline:
                    raise RuntimeError(f'initial context prefill timed out: {sid}')
                time.sleep(0.05)
            if managed_preload:
                while True:
                    state = utility('initial_context_status', f's{sid}e1')
                    if state['evicted_blocks']:
                        break
                    if time.monotonic() > deadline:
                        raise RuntimeError(f'initial context offload timed out: {sid}')
                    time.sleep(0.05)
                log.info('initial context session %d complete: %r', sid, state)
        ...
        if os.environ.get("OMNI_RETAINED_PREFIX_BLOCKS") and not os.environ.get('OMNI_RESIDENT_ONLY'):
