...
matched Metronome baseline 的第一方引擎代码，由 `experiments/baseline/` 按路径 spawn。

- `worker/stream_server.py` 从第三方 Metronome worker 分道后加入 parallel input processing、initial-context preloading 和共同观测。它当前每 segment 配置 \(M+8\) 的 decode cap；这是待修实验缺陷，不是 workload requirement。
