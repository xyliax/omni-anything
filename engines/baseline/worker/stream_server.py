...
Run through experiments/baseline with the environment built by infra/env/setup.sh:
...
Instrumentation (the writers live in infra/trace/collectors/worker_obs.py — the
one producer shared with conveyor; line grammars live with the consumers in
infra/trace/parse.py): PERREQ_LOG appends per-request event lines on one shared
...
# (infra/trace/parse.py excludes it from every statistic); a contract test pins
