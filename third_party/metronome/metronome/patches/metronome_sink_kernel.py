...
import os as _os
_SINK = int(_os.environ.get('METRONOME_SINK_TOKENS', '0') or '0')
...
def kernel_unified_attention_2d(
    ...
    SINK_TOKENS: tl.constexpr,  # int (Metronome CTX-token sink: keep first S keys)
    ...
    # iterate through tiles. With a CTX sink the union is [0,S) ∪ [t-W, t], so tiles
    # [0, SINK) must be visited in addition to the window range; extend the window's tile
    # range down to tile 0 and let the per-key mask (below) exclude the middle. Reading the
    # (freed/null) middle blocks is harmless: their keys are masked to -inf so they
    # contribute 0. Zero tile_start via multiplication so the loop bound stays Triton-typed
    # (a Python-int 0 start mixed with a Triton-value end miscompiles the loop).
    if SINK_TOKENS > 0:
        tile_start = tile_start * 0
    for j in range(tile_start, tile_end):
        ...
        if SLIDING_WINDOW > 0:
            if SINK_TOKENS > 0:
                # union [t-W, t] ∪ [0, S); use seq_offset[None, :] (2d) in both terms so
                # the sink term broadcasts to [BLOCK_M, TILE_SIZE] like the causal mask
                # above — a bare 1d seq_offset here mis-broadcasts and corrupts the mask.
                # union [t-W, t] ∪ [0, S); use seq_offset[None, :] (2d) in both terms so
                # the sink term broadcasts to [BLOCK_M, TILE_SIZE] like the causal mask.
                seq_mask = seq_mask & (
                    ((query_abs_pos - seq_offset[None, :]) < SLIDING_WINDOW)
                    | (seq_offset[None, :] < SINK_TOKENS)
                )
            else:
        ...
        if SLIDING_WINDOW:
            qpos_lo = q_block_local_idx * BLOCK_Q
            # This second, V-side window filter zeros keys older than the window. With a
            # CTX sink it must ALSO keep [0,S): otherwise sink keys carry softmax weight
            # (in L) but a zeroed V, under-normalizing the output. Mirror the seq_mask union.
            _v_in_win = (context_len + qpos_lo - seq_offset[:, None]) < SLIDING_WINDOW
            if SINK_TOKENS > 0:
                V = tl.where(_v_in_win | (seq_offset[:, None] < SINK_TOKENS), V, 0.0)
            else:
                V = tl.where(_v_in_win, V, 0.0)
