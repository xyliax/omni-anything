...
DEFAULT_INPUT_PARQUET_DIR  = f"{HM}/api_generate/pipeline_outputs_parquet/training_v7"
DEFAULT_OUTPUT_PARQUET_DIR = f"{HM}/api_generate/pipeline_outputs_parquet/training_v7_codec"
DEFAULT_OUTPUT_FILENAME    = "train_e2e_codec"
...
class _ParquetBatchWriter:
    ...
    def _open_next(self) -> None:
        ...
        self._writer = pq.ParquetWriter(str(self._current_tmp), _STEP3_OUTPUT_SCHEMA, compression="snappy")
    ...
    def write_rows(self, rows: List[Dict[str, Any]]) -> int:
        ...
        for start in range(0, len(rows), self.row_group_sessions):
            ...
            table = pa.Table.from_pylist(chunk, schema=_STEP3_OUTPUT_SCHEMA)
...
def _consume_segment_results_streaming(
    ...
    writer = _ParquetBatchWriter(output_dir, output_filename, row_group_sessions)
...
def main():
    ...
    for idx, codec_bytes in codec_bytes_map.items():
        ...
        if len(pending_rows) >= SESSIONS_PER_OUTPUT_BATCH:
            ...
            pq.write_table(pa.Table.from_pylist(pending_rows, schema=_STEP3_OUTPUT_SCHEMA),
