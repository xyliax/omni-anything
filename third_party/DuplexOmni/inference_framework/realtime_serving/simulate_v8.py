...
TEST_INPUT_AUDIO = os.environ.get("TEST_INPUT_AUDIO", "assets/input.wav")
OUTPUT_FILENAME = os.environ.get("OUTPUT_FILENAME", "outputs/simulate_output.wav")
...
S2_SYSTEM_PROMPT = """你是指导 System 1 说话的 System 2。
...
你可以根据上下文自由发挥，适当编造合理的细节来回应用户，不要说不知道或无法回答。
...
class System2Agent:
    ...
    async def trigger_think(self):
        ...
        async with self.lock:
            ...
            if new_context:
                ...
                logging.info(f"🧠 S2 Context Consolidated:\n{new_context}")
    ...
    async def _continuous_thinking(self):
        ...
        try:
            ...
            if not any(m.get("role") == "user" for m in messages_for_request):
                logging.warning("S2 skipping think: no user message in history yet")
                return
...
class OfflineSimulator:
    def __init__(self, user_audio_path):
        ...
        with wave.open(user_audio_path, 'rb') as wf:
            ...
            self.user_audio_pcm = np.frombuffer(frames, dtype=np.int16)
        if orig_sr != SAMPLE_RATE:
            import librosa
            audio_f32 = self.user_audio_pcm.astype(np.float32) / 32768.0
            audio_f32 = librosa.resample(audio_f32, orig_sr=orig_sr, target_sr=SAMPLE_RATE)
            self.user_audio_pcm = np.clip(audio_f32 * 32767, -32768, 32767).astype(np.int16)
            ...
            try:
                async with websockets.connect(ws_url, max_size=None) as ws:
                    ...
                    while True:
                        ...
                        arrival_s = max(0.0, time.monotonic() - float(self.sim_start_mono or time.monotonic()))
    ...
    async def run(self):
        ...
        try:
            s1_messages = [{"role": "system", "content": """你是有用的助手\n你的助手风格是：控场型。\n你的开场方式要求：先短答不展开。"""}]
            for i in range(self.total_chunks):
                target_send_time = float(self.sim_start_mono) + i * (CHUNK_MS / 1000.0)
                sleep_s = max(0.0, target_send_time - time.monotonic())
                if sleep_s > 0:
    ...
    async def _parse_and_execute_s1(self, text, chunk_idx):
        try:
            ...
            if "[THINK]" in s2_ctrl:
                logging.info(f"[{chunk_idx}] 🧠 Triggering S2 THINK")
...
def convert_to_wav_if_needed(audio_path):
    """Convert non-WAV audio to WAV using PyAV from megaswift env. Returns WAV path."""
    if audio_path.lower().endswith('.wav'):
        return audio_path
    wav_path = os.path.splitext(audio_path)[0] + '_converted.wav'
    if os.path.exists(wav_path):
        logging.info(f"Using cached converted WAV: {wav_path}")
        return wav_path
    logging.info(f"Converting {audio_path} -> {wav_path} ...")
    megaswift_python = os.environ.get("MEGASWIFT_PYTHON", "python")
    script = f"""
import av, wave, numpy as np, librosa
container = av.open({repr(audio_path)})
...
with wave.open({repr(wav_path)}, 'wb') as wf:
    wf.setnchannels(1)
    wf.setsampwidth(2)
    wf.setframerate({SAMPLE_RATE})
    wf.writeframes(pcm)
print(f'Converted {{len(pcm)}} bytes of PCM to {repr(wav_path)}')
"""
    import subprocess
    result = subprocess.run([megaswift_python, '-c', script], capture_output=True, text=True)
...
if __name__ == "__main__":
    TEST_INPUT_AUDIO = convert_to_wav_if_needed(TEST_INPUT_AUDIO)

    if os.path.exists(TEST_INPUT_AUDIO):
        preprocess_audio_noise_gate(TEST_INPUT_AUDIO)

    if not os.path.exists(TEST_INPUT_AUDIO):
        logging.warning("Generating dummy input audio...")
        dummy = np.random.randint(-500, 500, CHUNK_SAMPLES * 40, dtype=np.int16)
        with wave.open(TEST_INPUT_AUDIO, 'wb') as wf:
            wf.setnchannels(CHANNELS)
            wf.setsampwidth(SAMPWIDTH)
            wf.setframerate(SAMPLE_RATE)
            wf.writeframes(dummy.tobytes())

    sim = OfflineSimulator(TEST_INPUT_AUDIO)
    asyncio.run(sim.run())
