...
PYPI_CACHE_HOSTNAME="${PYPI_CACHE_HOSTNAME:-pypi-cache.local}"
mkdir -p ${builder_cache_dir}

# Try building the docker image
cat <<EOF | DOCKER_BUILDKIT=1 docker build \
    --add-host ${PYPI_CACHE_HOSTNAME}:${PYPI_CACHE_HOST} \
...
RUN pip config set global.index-url http://${PYPI_CACHE_HOSTNAME}:${PYPI_CACHE_PORT}/pypi/simple && \
    pip config set global.trusted-host ${PYPI_CACHE_HOSTNAME} && \
    apt-get update -y && \
