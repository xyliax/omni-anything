...
from vllm.model_executor.models.qwen3_omni_moe_code_predictor_mtp import (
...
class Qwen3OmniMoeTalkerForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Mirror HF talker init, but strip mrope keys from rope_parameters. HuggingFace
        # keeps mrope_section only in rope_scaling; promoting it into rope_parameters would
        # make vLLM ModelConfig.uses_mrope True after load and force the M-RoPE worker
        # path (Requires SupportsMRoPE + vision layout), which is wrong for this scaffold.
        _rs = getattr(talker_config.text_config, "rope_scaling", None) or {}
        _rp = dict(_rs) if isinstance(_rs, Mapping) else {}
        _rp["rope_theta"] = talker_config.text_config.rope_theta
        for _k in ("mrope_section", "mrope_interleaved", "interleaved"):
            _rp.pop(_k, None)
        talker_config.text_config.rope_parameters = _rp
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        # Qwen3MoeLLMModel.forward 在 PP 末段返回 **单个** hidden_states；非末段返回
        # IntermediateTensors。若误写 `h, _ = model(...)`，末段时右侧为 Tensor，Python 会按
        # dim0 迭代成行，再解包成两个变量 → seq>2 即 ValueError: too many values to unpack。
    ...
    def compute_logits(
        ...
        """
        # Stash rows that receive logits (typically the decode tail) so offline
        # servers can retrieve last hidden via LLM.apply_model without a second forward.
        self._talker_last_sample_hidden_cpu = hidden_states.detach().to(
            dtype=torch.float32
        ).cpu()
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Talker 无 Thinker 侧的 visual / audio_tower；此前误拷贝全模 load_weights 尾部会在 TP>1
        # 等路径下于 load_model 阶段触发 AttributeError。
        return loaded
