...
class TokensPrompt(_PromptOptions):
    ...
    """
    Optional per-position prompt hidden states. When provided together with
    `prompt_token_ids` (same length) and `enable_prompt_embeds` is set on the
    engine, vLLM uses token ids for RoPE/layout-sensitive paths and these rows
    as the actual decoder inputs (e.g. Qwen3 Omni talker bridge tensors).
    """
...
class TokenInputs(_InputOptions):
    ...
    prompt_embeds: NotRequired[torch.Tensor]
    """Optional CPU tensor of shape (len(prompt_token_ids), hidden_size)."""
