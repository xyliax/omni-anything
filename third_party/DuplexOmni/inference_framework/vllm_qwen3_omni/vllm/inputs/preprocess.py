...
class InputPreprocessor:
    ...
    @staticmethod
    def _coerce_config_token_id(value: object) -> int | None:
        """Normalize HF config token ids (int or list[int]) to a single int."""
        if value is None:
            return None
    ...
    def _bos_token_id_without_tokenizer(self) -> int | None:
        """Resolve BOS when ``skip_tokenizer_init=True`` from ``hf_config``."""
        hf = self.model_config.hf_config
        # Qwen3-Omni Talker (and similar): codec stream uses ``codec_bos_id``.
        codec_bos = getattr(hf, "codec_bos_id", None)
        if codec_bos is not None:
            return int(codec_bos)
        tid = self._coerce_config_token_id(getattr(hf, "bos_token_id", None))
        if tid is not None:
            return tid
        text_cfg = getattr(hf, "text_config", None)
        if text_cfg is not None:
            tid = self._coerce_config_token_id(getattr(text_cfg, "bos_token_id", None))
            if tid is not None:
                return tid
        logger.warning_once(
            "Using None for BOS token id: tokenizer not initialized and "
            "hf_config has no codec_bos_id / bos_token_id"
        )
        return None

    def _eos_token_id_without_tokenizer(self) -> int | None:
        """Resolve EOS when ``skip_tokenizer_init=True`` from ``hf_config``.

        For codec-talker models, stopping must match **codec** EOS (e.g.
        ``codec_eos_token_id``), not only the text LM ``eos_token_id``, so
        ``Request.eos_token_id`` aligns with ``check_stop`` and sampling.
        """
        hf = self.model_config.hf_config
        codec_eos = getattr(hf, "codec_eos_token_id", None)
        if codec_eos is not None:
            return int(codec_eos)
        tid = self._coerce_config_token_id(getattr(hf, "eos_token_id", None))
        if tid is not None:
            return tid
        text_cfg = getattr(hf, "text_config", None)
        if text_cfg is not None:
            tid = self._coerce_config_token_id(getattr(text_cfg, "eos_token_id", None))
            if tid is not None:
                return tid
        logger.warning_once(
            "Using None for EOS token id: tokenizer not initialized and "
            "hf_config has neither codec_eos_token_id nor eos_token_id"
        )
        return None

    def get_bos_token_id(self) -> int | None:
        if self.tokenizer is None:
            return self._bos_token_id_without_tokenizer()
        return self.tokenizer.bos_token_id

    def get_eos_token_id(self) -> int | None:
        if self.tokenizer is None:
            return self._eos_token_id_without_tokenizer()
    ...
    def _process_tokens(
        ...
        *,
        ...
        if multi_modal_data := parsed_content.get("multi_modal_data"):
            ...
            if "prompt_embeds" in parsed_content:
                raise ValueError(
                    "prompt_embeds with multi_modal_data is not supported."
                )
        else:
            opt_embeds = parsed_content.get("prompt_embeds")
            if opt_embeds is not None:
                if not self.model_config.enable_prompt_embeds:
                    raise ValueError(
                        "You must set `--enable-prompt-embeds` when passing "
                        "prompt_embeds with prompt_token_ids."
                    )
                pe = opt_embeds
                if pe.ndim == 3:
                    pe = pe.squeeze(dim=0)
                ...
                if pe.shape[0] != len(prompt_token_ids):
                    raise ValueError(
                        "prompt_embeds seq_len must match len(prompt_token_ids): "
                        f"{pe.shape[0]} vs {len(prompt_token_ids)}"
                    )
                inputs = token_inputs(prompt_token_ids)
                inputs["prompt_embeds"] = pe.cpu().contiguous()
            else:
                inputs = token_inputs(prompt_token_ids)
    ...
    def _prompt_to_llm_inputs(
        ...
        * [`SingletonInputs`][vllm.inputs.data.SingletonInputs] instance
        """
        # Token ids + embeds (same length): talker / omni bridge; must run before
        # embed-only branch so both tensors are forwarded.
        if "prompt_token_ids" in prompt and "prompt_embeds" in prompt:
            return self._process_tokens(
                cast(TokensPrompt, prompt),
                tokenization_kwargs=tokenization_kwargs,
                mm_uuids=mm_uuids,
            )
