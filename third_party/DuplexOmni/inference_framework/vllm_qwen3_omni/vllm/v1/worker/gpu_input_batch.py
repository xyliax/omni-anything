...
class InputBatch:
    ...
    def add_request(
        ...
        if request.prompt_token_ids is not None:
            ...
            # Hybrid prompts: when every prompt row has a parallel `prompt_embeds` row,
            # those embeddings are authoritative. Leaving `is_token_ids=True` would
            # make `GPUModelRunner._prepare_inputs` call `embed_input_ids` on all
            # prompt positions and overwrite `inputs_embeds` (see gpu_model_runner).
            if request.prompt_embeds is not None:
                pe_len = int(request.prompt_embeds.shape[0])
                if pe_len == num_prompt_tokens:
