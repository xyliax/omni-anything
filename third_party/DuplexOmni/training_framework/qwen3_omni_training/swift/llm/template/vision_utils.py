...
def _wrap_pcm_as_wav(data: bytes, sample_rate: int = 24000, channels: int = 1, bits: int = 16) -> BytesIO:
    import struct
    data_len = len(data)
    header = struct.pack(
        '<4sI4s4sIHHIIHH4sI',
        ...
        b'data', data_len)
    return BytesIO(header + data)


def load_audio(audio: Union[str, bytes], sampling_rate: int, return_sr: bool = False):
    import librosa
    if isinstance(audio, (bytes, bytearray)) and not bytes(audio[:4]) == b'RIFF':
        audio_io = _wrap_pcm_as_wav(bytes(audio))
    else:
