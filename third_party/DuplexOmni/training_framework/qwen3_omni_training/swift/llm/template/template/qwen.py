...
class Qwen3OmniE2ETemplate(Qwen3OmniTemplate):
    fixed_speaker_id = 2301
    accept_hidden_layer = 24
    num_code_groups = 16

    @classmethod
    def _normalize_rvq_codes(cls, payload: Any, codec_path: str) -> torch.Tensor:
        codes = payload.long()
        if codes.ndim == 1:
            codes = codes.unsqueeze(0)
        elif codes.ndim == 3:
            if codes.shape[0] != 1:
                raise ValueError(f'Unsupported RVQ batch shape: {tuple(codes.shape)} in {codec_path}')
            codes = codes[0]
        if codes.ndim != 2:
            raise ValueError(f'Unsupported RVQ code shape: {tuple(codes.shape)} in {codec_path}')
        num_code_groups = cls.num_code_groups
        if codes.shape[0] == num_code_groups and codes.shape[1] != num_code_groups:
            return codes.contiguous()
        if codes.shape[1] == num_code_groups and codes.shape[0] != num_code_groups:
            return codes.transpose(0, 1).contiguous()
        if codes.shape[0] == num_code_groups and codes.shape[1] == num_code_groups:
            return codes.contiguous()
        raise ValueError(f'Cannot infer RVQ layout for shape {tuple(codes.shape)} in {codec_path}')

    @staticmethod
    def _format_codec_source(codec_source: Any) -> str:
        if isinstance(codec_source, (bytes, bytearray, memoryview)):
            return f'<inline codec bytes len={len(codec_source)}>'
        return str(codec_source)

    @classmethod
    def _load_codec_payload(cls, codec_path: Any) -> Dict[str, torch.Tensor]:
        codec_label = cls._format_codec_source(codec_path)
        if isinstance(codec_path, (bytes, bytearray, memoryview)):
            codec_path = io.BytesIO(bytes(codec_path))
        payload = torch.load(codec_path, map_location='cpu')
        if not isinstance(payload, dict):
            payload = {'codes': payload}
        code_payload = payload.get('codes', payload.get('codec', payload.get('code', None)))
        if code_payload is None:
            raise ValueError(f'Invalid codec file, missing `codes`: {codec_label}')
        codes = cls._normalize_rvq_codes(code_payload, codec_label)
        total_len = int(codes.shape[-1])

        boundaries_raw = payload.get(
            'turn_boundaries',
            payload.get('boundaries', payload.get('turn_ends', payload.get('turn_offsets', None))))
        turn_lengths = payload.get('turn_lengths')
        if boundaries_raw is None and turn_lengths is not None:
            lengths = torch.as_tensor(turn_lengths, dtype=torch.long).reshape(-1)
            boundaries = torch.cat([lengths.new_zeros(1), lengths.cumsum(dim=0)], dim=0)
        elif boundaries_raw is None:
            boundaries = torch.tensor([0, total_len], dtype=torch.long)
        else:
            boundaries = torch.as_tensor(boundaries_raw, dtype=torch.long).reshape(-1)
            if boundaries.numel() == 0:
                boundaries = torch.tensor([0, total_len], dtype=torch.long)
            elif int(boundaries[0].item()) != 0:
                boundaries = torch.cat([boundaries.new_zeros(1), boundaries], dim=0)
        if int(boundaries[-1].item()) != total_len:
            if int(boundaries[-1].item()) < total_len:
                boundaries = torch.cat([boundaries, boundaries.new_tensor([total_len])], dim=0)
            else:
                raise ValueError(
                    f'Invalid codec boundaries in {codec_label}: last boundary={int(boundaries[-1].item())}, total_len={total_len}')
        if boundaries.numel() < 2:
            raise ValueError(f'Codec boundaries must contain at least one turn in {codec_label}')
        if not torch.all(boundaries[1:] > boundaries[:-1]):
            raise ValueError(f'Codec boundaries must be strictly increasing in {codec_label}: {boundaries.tolist()}')
        turn_count = int(boundaries.numel() - 1)

        assistant_turn_indices_raw = payload.get(
            'assistant_turn_indices',
            payload.get('turn_message_indices', payload.get('message_indices', payload.get('turn_indices', None))))
        if assistant_turn_indices_raw is None:
            assistant_offset = int(payload.get('assistant_offset', payload.get('turn_offset', 0)))
            assistant_turn_indices = torch.arange(turn_count, dtype=torch.long) + assistant_offset
        else:
            assistant_turn_indices = torch.as_tensor(assistant_turn_indices_raw, dtype=torch.long).reshape(-1)
            if assistant_turn_indices.numel() != turn_count:
                raise ValueError(
                    f'Codec assistant_turn_indices mismatch in {codec_label}: '
                    f'turn_count={turn_count}, indices={assistant_turn_indices.numel()}')

        return {
            'codes': codes,
            'turn_boundaries': boundaries,
            'assistant_turn_indices': assistant_turn_indices,
        }

    @staticmethod
    def _collate_rvq_batch(batch: List[Dict[str, Any]], codes_key: str, length_key: str) -> Dict[str, torch.Tensor]:
        rvq_codes = [b[codes_key] for b in batch if b.get(codes_key) is not None]
        if not rvq_codes:
            return {}
        rvq_codes = [c if isinstance(c, torch.Tensor) else torch.tensor(c, dtype=torch.long) for c in rvq_codes]
        max_len = max(c.shape[-1] for c in rvq_codes)
        padded = []
        lengths = []
        for codes in rvq_codes:
            lengths.append(codes.shape[-1])
            if codes.shape[-1] < max_len:
                codes = F.pad(codes, (0, max_len - codes.shape[-1]), 'constant', 0)
            padded.append(codes)
        return {
            codes_key: torch.stack(padded, dim=0),
            length_key: torch.tensor(lengths, dtype=torch.long),
        }

    @staticmethod
    def _normalize_codec_turn_meta_1d(meta: Any, *, name: str) -> torch.Tensor:
        meta = meta if isinstance(meta, torch.Tensor) else torch.tensor(meta, dtype=torch.long)
        if meta.ndim == 1:
            return meta.long().contiguous()
        if meta.ndim == 2 and meta.shape[0] == 1:
            logger.info_once(
                f'qwen3_omni_e2e: squeeze singleton leading dim for {name} in collator.',
                hash_id=f'qwen3_omni_e2e_{name}_singleton_collator')
            return meta[0].long().contiguous()
        raise ValueError(
            f'qwen3_omni_e2e expects {name} to be rank-1 [turns] or rank-2 [1, turns], '
            f'got shape={tuple(meta.shape)}')

    @classmethod
    def _collate_turn_meta_batch(cls, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        boundaries = [b.get('codec_turn_boundaries') for b in batch]
        assistant_turn_indices = [b.get('codec_assistant_turn_indices') for b in batch]
        if any(v is None for v in boundaries) or any(v is None for v in assistant_turn_indices):
            raise ValueError('qwen3_omni_e2e batch contains samples without codec turn metadata.')
        boundaries = [cls._normalize_codec_turn_meta_1d(v, name='codec_turn_boundaries') for v in boundaries]
        assistant_turn_indices = [
            cls._normalize_codec_turn_meta_1d(v, name='codec_assistant_turn_indices') for v in assistant_turn_indices
        ]
        turn_counts = []
        max_turns = 0
        for boundary, indices in zip(boundaries, assistant_turn_indices):
            turn_count = int(indices.numel())
            if boundary.numel() != turn_count + 1:
                raise ValueError(
                    f'codec turn metadata mismatch after normalization: '
                    f'boundaries={tuple(boundary.shape)}, indices={tuple(indices.shape)}, '
                    f'boundary_values={boundary.tolist()[:8]}, index_values={indices.tolist()[:8]}')
            turn_counts.append(turn_count)
            max_turns = max(max_turns, turn_count)
        padded_boundaries = []
        padded_indices = []
        for boundary, indices, turn_count in zip(boundaries, assistant_turn_indices, turn_counts):
            boundary_pad = max_turns + 1 - boundary.numel()
            index_pad = max_turns - indices.numel()
            padded_boundaries.append(F.pad(boundary, (0, boundary_pad), 'constant', -1))
            padded_indices.append(F.pad(indices, (0, index_pad), 'constant', -1))
        return {
            'codec_turn_boundaries': torch.stack(padded_boundaries, dim=0),
            'codec_assistant_turn_indices': torch.stack(padded_indices, dim=0),
            'codec_turn_count': torch.tensor(turn_counts, dtype=torch.long),
        }

    def _attach_talker_teacher_forcing(self,
                                       encoded: Dict[str, Any],
                                       codec_path: Any,
                                       assistant_total_count: int) -> Dict[str, Any]:
        input_ids = encoded['input_ids']
        ...
        if labels is not None:
            labels_t = torch.tensor(labels, dtype=torch.long) if isinstance(labels, list) else labels
            if labels_t.ndim == 2:
                labels_t = labels_t[0]
                encoded['labels'] = labels_t
        if codec_path is None or (isinstance(codec_path, str) and not codec_path) or (
                isinstance(codec_path, (bytes, bytearray, memoryview)) and len(codec_path) == 0):
            raise ValueError('qwen3_omni_e2e training requires every sample to provide a non-empty `codec`.')
        codec_payload = self._load_codec_payload(codec_path)
        codec_turn_boundaries = self._normalize_codec_turn_meta_1d(
            codec_payload['turn_boundaries'], name='codec_turn_boundaries')
        codec_assistant_turn_indices = self._normalize_codec_turn_meta_1d(
            codec_payload['assistant_turn_indices'], name='codec_assistant_turn_indices')
        encoded['rvq_codes'] = codec_payload['codes']
        encoded['rvq_length'] = torch.tensor(codec_payload['codes'].shape[-1], dtype=torch.long)
        encoded['codec_turn_boundaries'] = codec_turn_boundaries
        encoded['codec_assistant_turn_indices'] = codec_assistant_turn_indices
        encoded['codec_turn_count'] = torch.tensor(codec_assistant_turn_indices.numel(), dtype=torch.long)
        encoded['assistant_total_count'] = torch.tensor(assistant_total_count, dtype=torch.long)
        encoded['talker_accept_hidden_layer'] = self.accept_hidden_layer
        encoded['speaker_id'] = self.fixed_speaker_id
        return encoded

    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        encoded = super()._encode(inputs)
        codec_path = inputs.extra_kwargs.get('codec', inputs.extra_kwargs.get('code'))
        assistant_total_count = sum(1 for msg in inputs.messages if msg.get('role') == 'assistant')
        return self._attach_talker_teacher_forcing(encoded, codec_path, assistant_total_count)

    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        res = super()._data_collator_mm_data(batch)
        if any(b.get('rvq_codes') is None for b in batch):
            raise ValueError('qwen3_omni_e2e batch contains samples without `rvq_codes`; mixed supervision is not supported.')
        res.update(self._collate_rvq_batch(batch, 'rvq_codes', 'rvq_length'))
        res.update(self._collate_turn_meta_batch(batch))
        speaker_ids = [b.get('speaker_id') for b in batch if b.get('speaker_id') is not None]
        if speaker_ids:
            res['speaker_id'] = torch.tensor(speaker_ids, dtype=torch.long)
        accept_hidden_layer = [b.get('talker_accept_hidden_layer') for b in batch if b.get('talker_accept_hidden_layer') is not None]
        if accept_hidden_layer:
            res['talker_accept_hidden_layer'] = torch.tensor(accept_hidden_layer, dtype=torch.long)
        assistant_total_count = [b.get('assistant_total_count') for b in batch if b.get('assistant_total_count') is not None]
        if assistant_total_count:
            res['assistant_total_count'] = torch.tensor(assistant_total_count, dtype=torch.long)
        return res


register_template(
    QwenTemplateMeta(
        MLLMTemplateType.qwen3_omni_e2e,
        template_cls=Qwen3OmniE2ETemplate,
        default_system=None,
