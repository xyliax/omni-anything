...
> **2026-08-06 status:** this is the design-history document. E2/E3 mechanism evidence rejects the
> old `2x` and continuous-rotation forecasts for the retained Qwen2.5-Omni/RTX-3090 point; the
> measured compute-constrained result is about `+5.1%`. See `FINDINGS.md`. End-to-end active-block
...
| 本仓库 3090+PCIe3（实测 12.33GB/s，`experiments/e0_dma_interference/reference_data/pcie_h2d_bench.json`），480ms | **+83%**（池仅 4.97GB，相对带宽富裕） | 原型验证信噪比最高 |
...
1. **模拟器扩展**：加链路资源 + H2D 调度器（标定现成：pinned H2D 12.33GB/s、4k swap 38.1ms、多尺寸 64MB–1GB 平坦——`experiments/e0_dma_interference/reference_data/pcie_h2d_bench.json`，2026-08 GPU3 实测）。
