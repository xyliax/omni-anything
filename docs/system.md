...
按上限规划不要求所有最大历史同时常驻 GPU，仍通过周期内的逐出与恢复复用空间。它省去对未来增长的预测与滚动续期，但可能少接纳一些当前历史较短的会话；这是当前选择的简单性与容量利用率取舍。该选择由本轮保守候选检查实现；它不是已验证的性能保证。
...
首轮采用[保守候选检查](#conservative-admission)，成本由独立校准提供。最大上下文约束只限定状态规模，周期完成表现仍由实验测量，不由该上限单独保证。
...
| 问题 | 所需决策或验证 |
| --- | --- |
| 保守候选检查（本轮采用） | 固定组数、既有 phase 与逐会话上限，按最大上下文检查完整目标时间线；成本由独立校准提供，不追求最优分组 |
| 预算与恢复顺序（本轮采用） | 采用统一固定 ceiling；按计划恢复起点排序，同起点保持会话顺序；只逐出确认已备份且无竞争引用的完整块 |
...
可按上述接口、状态区分、恢复分配与修正规则实现并验证执行路径。本轮候选检查、固定 ceiling 与恢复排序按[保守策略](#conservative-admission)执行，成本余量仍须通过独立校准确定，不能从图中的四组、等间距示意或箭头先后顺序推定默认答案。主动周期性重算按[future work](#future-recomputation)处理，不扩入当前 H2D 主路径。
...
当前保守实现以显式配置的 `restore_lead_s` 指定窗口长度，planner 用整组逐出上限和成本 profile 检查其可行性；Session Manager 在 `next_tick - restore_lead_s` 到达后检查空间并提交恢复。它尚未根据每周期实际字节自动缩短或重新标定启动提前量。逐出上限限制后续 H2D 的工作量，不是到时强行截断在途 DMA；超窗时依然等待有效状态，并记录完成迟到。配置预测可行与实测按时完成必须分别验收，旧有限前瞻路径的低频 review 不代替成本标定；本轮最大上下文路径只在会话变化时修正。
...
作者选择以可解释的保守策略完成首轮评估，不要求先实现一般联合优化器。固定周期、组数、恢复提前量与逐会话逐出 ceiling，保持已有 phase 和预算；先按组成员数，再按相对首次源输入齐备的对齐等待、最后按 slot 编号尝试新成员位置。计数只决定搜索顺序，每个候选仍须通过资源检查。

最大历史量从实际后端最大上下文和 block 几何读取，包含输入与生成，不使用增长预测或滚动期限。主机按所有成员的最大历史收费。每组计算使用按成员数标定的成本表，缺项退回逐成员成本之和；增量备份按成员数累加，与计算保守串行占用组间隔；全部成员的恢复 ceiling 共享同一恢复窗口，扣除提交与安全余量后检查有效服务量。成本须覆盖对应工作量、执行方式及传输干扰，未标定 profile 不构成服务能力证据。

GPU 预测采用周期区间扫描：每会话始终收取最大历史减逐出 ceiling；从恢复窗口开始完整收取目的分配，到计算和备份预算结束才计入逐出收益。时间相同先计分配再计释放。实际引用占用超出当前相位估计时，将差额作为整周期额外余量。GPU 预留不得小于实际分配器的恢复门槛；潜在共享不作为准入收益。新历史只按实际生成分配，实际逐出量受已完成备份、完整块和非共享引用约束，不会为了符合预测丢弃历史。

会话变化触发候选检查与版本递增，上限内增长不续期。切换先暂停新 H2D 分配，等待已分配复制结算，再在同一资源锁内重读实际占用并发布计划；既有执行和增量备份可继续。已发起操作仍按独立身份完成资源结算。当前实现只增删成员，不修改既有 phase 或 ceiling；不宣称已实现任意预算重排。

恢复按计划起点排序，完整目的分配失败时停止后续定时恢复；已有复制继续。每会话有效状态独立发布，输入 readiness 保留到后端取得执行引用。恢复窗口结束不截断 DMA；迟到仍等待有效历史，并由实际完成观测结算。

评估回放保持源时钟和已确定 tick；不可行新会话直接拒绝，只有资源切换中的控制操作暂待完成。全驻留和按需回载使用独立校准后固定的计数准入上限；计数控制不使用主动 phase 预测的容量信用。按需回载在 CPU 输入前处理结束、执行需要历史时触发；提交后预取与 CPU 前处理重叠；提前恢复由独立定时器在下一 tick 前触发。三者共用有效性和引用保护，不把提前分配等同于就绪。

结束分为排空与资源归还：最后输入完成且前端收到固定预算输出后，取消持久请求并等待所有复制引用释放，planner 才删除 reservation，gateway 才确认完成。异常断连不能冒充完成；取消幂等，旧输入或计划不能复活已关闭身份。输入/生成规则改变时须重新核验排空依据。

旧有限前瞻检查器和准入后播放的 FIFO 客户端仍只服务历史诊断，开放回放拒绝该成本模式。一般异质需求、自动组数/预算搜索、任意计划重排及成本越界时的优化恢复仍待后续工作；这些不是首轮评估必须具备的机制。可执行设置、控制组约束及复现入口见[实验配置](experiments.md)。
