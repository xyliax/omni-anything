...
Pilarius 探索如何利用周期负载承诺的下一次使用时刻（[问题定义](problem.md#why-timing-information-matters)），在更新间减少 GPU KV 驻留，并把恢复安排到后续计算需要之前。把这份信息转化为容量收益，设计需要同时处理三个挑战。(1) **需求落在周期的哪个位置**：时间信息是每会话的，资源是共享的；若各会话的周期需求相位对齐，恢复期限与活跃驻留在时间轴上同时叠加，链路与 GPU 峰值随会话数抬升——单会话的可预测性不自动带来多会话的可行性。(2) **空闲期回收多少**：回收驻留要求移除 GPU 内容，参考语义却要求被移除的状态在下次使用前完整重建；逐出的合法域、深度与副本有效性必须可判定，而建立副本本身消耗同一条传输链路。(3) **使用前何时恢复**：恢复过晚即回到关键路径，退化为事后触发的按需恢复；过早则重新占用刚回收的空间并与其他会话竞争传输——就绪时机落在两侧都有代价的窗口内，且实际执行时刻仍受排队与批组成扰动。三个挑战共享同一资源耦合：偏移指派同时决定各会话的链路时间份额、空间占用时长与批组成（→ [资源边界](problem.md#resource-frontier)）。它们也分别对应利用时间信息三级所须付出的代价：回收只依赖节奏存在，恢复时机还需释放时刻可见，需求安置则以偏移可控为前提（→ [时间信息的机会与代价](problem.md#why-timing-information-matters)）。
