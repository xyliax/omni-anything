...
双工 serving 在真机上是 capacity-bound（因 KV 池装不下而失败，区别于 bandwidth-bound 意义上的 memory-bound）：池打满即多数会话 starve，而同一时刻 GPU 约有一半时间空闲（FINDINGS C1、D1）。
tick 内的 H2D 链路同样大量闲置，本设计的全部内容就是用这两份闲置资源赎回 HBM 容量。
KV conveyor：按 tick 时间表把每路会话的尾部 KV 母本 (canonical copy) 从 host DRAM 经 DMA 预取入 HBM 暂存、算完即释放（scheduled tail-KV offloading，谱系为 FlexGen / InfiniGen / vLLM offloading connector / LMCache），等效 KV 容量 = resident M + staging P。
...
机制从「重算」换成「搬运」，换掉的是链路而非算力：单价从约 0.035ms/token 引擎步时间降到约 2µs/token DMA 时间，走 copy engine、与计算并行、不污染 batch 组成。
...
尾部累计到阈值 C 就 commit 为 resident，于是每 tick 的搬运量有界，长会话不会把链路吃穿。
...
| b | 每 token 的 KV 字节 | 56 KiB（Qwen2.5-Omni-7B thinker：2×28 层×4 KV 头×128×2B）；1.7B 实测 112KB 反而更大（其 GQA KV 头 8，为 7B 的两倍） |
...
| X* | 使可调度并发数最大的 X | 示例 ≈3.1k token（180MB/路） |
| C | commit 阈值 | C ≈ 2X*（示例 6–8k） |
| a | 每路每 tick 新生 token 数 | ≈10（480ms tick）；本机 2s tick 实测约 78（53 音频 + 约 25 生成） |
| η | 链路可用率 | 工程假设 0.7；E0 实测计算期可用率约 0.94 |
| B_link | H2D 可用带宽 | 本机实测 12.30–12.33GB/s（E0 复测与 `calibration/data/pcie_h2d_bench.json`，Gen3 档）；示例 PCIe5 约 40GB/s |
| T | tick 长（硬 tick，固定周期 T 的硬 deadline 帧，即 Metronome 的 frame budget B） | 本机 E 系列 2s；示例 480ms |
| N / N* | 并发路数 / 可调度并发数 (schedulable concurrency，判据 miss rate ≤ 1%) | 见收益表 |
...
代入 H100 级示例配置（PCIe5 / 480ms / L=16k）：X*≈3.1k token（180MB/路），C≈6–8k，t_x=6.4ms，t_c=10–30ms，staging 峰值 N·X·(t_x+t_c+τ_lead)/T ≈ 1.8GB（占池 3%，已计入净收益）。

## 编排

η 不是硬件给的，是调度挣的：随机相位下需求聚簇造成链路瞬时尖峰，要么 H2D 迟到（miss），要么被迫压低 η（收益缩水）。设计目标是把 H2D 需求摊平成近恒定速率。
...
- 时间排他性是容量扩展的必要条件：同步 tick 下 conveyor 无收益，因为忙碌窗内每个 step 都读全员 KV，峰值 resident 不降（D3）。相位打散 (desync) 用算力余量换常驻容量，代价是权重每 step 重读，守恒律 B̄ ≥ N×配额×t_step/T ≈ 2.8，上界 T·BW/(配额×W) ≈ 3.4（3090 + 7B）。
...
公平性设计：主对比在同一 worker 内做 conveyor on vs off，唯一差异是 KV 住哪；vanilla vLLM realtime 路径作参照 baseline (sanity baseline)，证明自研 worker 的绝对性能不虚；windowed KV 作有损竞争方案对照。
...
| MoshiRAG（ICML'26） | 模型层回避：弃 insertive 注入改加法叠加，"to constrain sequence length" | 模型侧为 serving 缺位买单的直接证据；本设计的 commit 语义正面接住 insertive 注入 |
...
- 并行性已实测：E0 二十格网格上 κ（decode step 时间膨胀系数，κ = t_with/t_without）最大 1.067，大格 1.01–1.03，干扰按每 step 加性约 2.5ms 建模而非乘性折损（FINDINGS E3）。DMA 与 decode 重叠这一物理前提在本机成立。
