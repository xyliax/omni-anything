...
> **2026-08-06 execution update:** E2/E3 now have trace-driven CUDA mechanism evidence. They use
> the successful E1 Qwen2.5-Omni trace and fresh pinned-H2D measurements, but do not yet change
> vLLM active-block ownership. The measured result is approximately `+5.1%`, not the earlier `2x`
> forecast. Fine-grained batch-about-3 rotation is compute-infeasible on this retained profile;
> groups 6/7 are the useful boundary. Current conclusions and exact evidence pointers are in
> [`FINDINGS.md`](FINDINGS.md). Older predictions below are retained as design history, not facts.
...
| `bench/gpu_probe.py` 的 `wait_for_window` + 本仓库 `experiments/e1_capacity_bottleneck/wait_for_gpu.sh` | 共享 GPU 空闲检测（防污染） |
| `worker/stream_server.py` | 参照实现（vanilla baseline 直接用它 + 文本分支约 20 行） |
| FIX 1/4（通用 vLLM bug） | 仅 vanilla baseline 需要；先查上游是否已修 |

改动纪律：第三方 pin 保持只读；我们的 worker、gateway 补丁、实验脚本全部放本仓库对应的
`experiments/<experiment>/`，gateway 改动以补丁文件管理。
...
- **方法**（工具为独立的 `experiments/e0_dma_interference/run.py`）：在 T1 网格子集
...
1. **每点新起进程（fresh-per-point）**：每个数据点新起 worker 进程（Metronome 与本仓库
   `run_all.sh` 双重教训）。
2. **共享 GPU 空闲检测**：每次测量前 `experiments/e1_capacity_bottleneck/wait_for_gpu.sh` + `gpu_probe.wait_for_window`；记录期间
...
4. **双重 miss 判据**：worker 自报 step time + 客户端节奏完整度（交付率 deliv_pct ≥ 0.9）
   交叉验证。
5. **工件纪律**：每个当前数字可追溯到 `results/<experiment>/runs/<run-id>/` 下的原始证据 + 生成脚本 + git 版本
...
冒烟通过：GPU3 / N=1 / 30s / 2s tick，15/15 帧节奏 100%，miss 0%，零漂移。该一次性 smoke
产物已在结果精简时删除；它不支撑当前论文结论。

**多会话协同**：另一会话（job bdc51189）在 GPU1 执行 E1 vanilla 配置扫描。本会话让出 E1，转执行 E0。
跨会话进程隔离约定：worker 用独立文件名副本 + `VLLM_PROCESS_NAME_PREFIX=<jobid>`，
勿用 `pkill -f` 模式杀（已发生两次误杀）。

**E0 结果（`results/e0_dma_interference/runs/20260804_initial/`，3090 / GPU3 / Qwen3-1.7B fp16）**：
...
当前统一入口：`experiments/e1_capacity_bottleneck/run.sh --mode vanilla`；进程组由 runner 定向清理。

**300s 初扫（N ∈ {2,4,6,8}，历史探索，不作为当前证据集）**：
...
latency 钉在 1600ms = worker `wait_budget`（0.8×tick）轮询顶，属于实验客户端的截断读数，
真实引擎 step time 更长；帧交付率同窗从 40 掉到 32。

**600s 补拍结论**：当前仅保留有效的
`results/e1_capacity_bottleneck/runs/vanilla_n8_d600/`；N=6 长跑受 MML 假稳定污染，已清理。
...
**E1 状态：初版完成**。早期粗扫描已经由精选的 vanilla 长跑、逐请求控制点和 paringest
重复实验取代；无效或被替代的原始产物不再保留。
...
此前只是记账反推。**历史插桩**：metronome worker 的仅加日志副本
克隆零改动）——P（tick 推送）/ F（引擎接收分片）/ T（输出增长 + 引擎侧 nprompt）三类事件共用一个时钟；
统计日志加 `pre=` 累计 preempt 数（`IterationStats.num_preempted_reqs`）。该观测不再是独立 mode；
当前运行器使用 `--mode vanilla|paringest` 和正交的 `--trace`。保留产物位于
`results/e1_capacity_bottleneck/runs/vanilla_n8_d600_request_trace/`。
...
- **测量学注记**：AsyncLLM 前端输出合批（374 个 T 事件覆盖 28.8k token），T 时间戳不可用于
  ...
  （decode 循环自持，天然可测）。
- **服务时序发现（`results/e1_capacity_bottleneck/runs/vanilla_n8_d600_request_trace/derived/service_timeline.png`，脚本
  `experiments/e1_capacity_bottleneck/analysis/plots/plot_e1_service_timeline.py`）**：输入端零相位——gateway 每 tick 一个批量 Step，
  ...
  + 干净主机复测 t_chunk。**N=4/6 对照已完成（`vanilla_n{4,6}_d300_request_trace`，GPU3/300s）：模型全中**——
...
### 2026-08-04：并行 ingest 补丁实验（paringest，N=8/600s/GPU3）——ingest 瓶颈证毕即除，capacity bottleneck 改换形态

**补丁**：`experiments/e1_capacity_bottleneck/workers/parallel_ingest.py`——用 monkeypatch 改写
...
虚拟环境与克隆零改动。当前入口为 `experiments/e1_capacity_bottleneck/run.sh --mode paringest`。

**修复验证（一行补丁的因果证明）**：积压全程恒 1（含崩溃期，零轮转）；F−P 从 1015ms → **3ms**
（p95 9ms）；独立 full-trace 重复的时间线（`results/e1_capacity_bottleneck/runs/paringest_n8_d600_full_trace_r2/derived/timeline.trace.json.gz`）显示
...
**新故障形态：capacity cascade（`results/e1_capacity_bottleneck/runs/paringest_n8_d600_request_trace_r1/derived/cascade_anatomy.png`，脚本
`experiments/e1_capacity_bottleneck/analysis/plots/plot_e1_cascade.py`）**：池打满 kv=1.000 → preempt 一个会话 → 仍在运行的会话以
...
只 starve 被 preempt 的那几路。**即 vanilla request-trace run 的 run=0/wait=8 全员死锁 = ingest 瓶颈 × capacity bottleneck 叠在一起的产物**；
...
### 2026-08-05：tick 内执行剖面实测（历史中间观测，结论由后续 full trace 接替）

**方法**：当时通过 `PERITER_LOG` 记录每引擎 step 的 t/run/gen/ptok。该短点已被随后更高分辨率的
`paringest_n8_d60_full_trace` 完全替代，因此原始 run 已从精选结果中清理。
...
忙碌窗合计 811ms/2000ms，其余纯空闲；这些结论由保留的 scheduler full trace 重新验证。
...
### 2026-08-05：scheduler 级逐步追踪（paringest + trace，N=8/60s）——tick 内每请求执行泳道，全部猜测清零

**方法**：`observability/vllm_scheduler_trace/sitecustomize.py` 经 PYTHONPATH 注入 EngineCore 子进程
（前端 monkeypatch 到不了 scheduler 所在进程；仅 `OMNI_SCHEDULER_TRACE` 设置时激活），包裹
`Scheduler.schedule` 逐步记录每请求的调度 token 数与 encoder 输入标记；请求 ID 自带 s{sid}
前缀直接映射会话。当前入口为 `run.sh --mode paringest --trace`；图
`results/e1_capacity_bottleneck/runs/paringest_n8_d60_full_trace/derived/execution_lanes.png`（脚本 `analysis/plots/plot_e1_lanes.py`），
...
决定链：runner 的 `--tpt 25` → worker 给每个分片的 SamplingParams 设 `max_tokens=25n+8` →
...
的差额在 worker 文本缓冲累积（无害但存在，runner 语义备忘）。
...
### 2026-08-06：capacity cascade 重复 2/5（paringest_n8_d600_full_trace_r2）——确定性时钟、被 preempt 会话任意
...
**cascade 泳道图** `results/e1_capacity_bottleneck/runs/paringest_n8_d600_full_trace_r2/derived/cascade_lanes.png`
（脚本 `experiments/e1_capacity_bottleneck/analysis/plots/plot_e1_cascade_lanes.py`）：
