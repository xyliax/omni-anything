...
这两种设计的共同启发可表述为：**让需要长期保留的信息在不同载体上维持，并按使用节奏复用有限的昂贵资源。** 延迟线的物理脉冲间隔和再生电路决定其串行节奏；现代设计的状态访问契约、传输与计算成本决定可回收和准备的机会。抛球展示了“保留状态总量可以大于同时 GPU 驻留量”的可能性，但具体收益仍须通过资源模型与实验检验。
