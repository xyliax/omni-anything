...
`C_N(L)` 表示指定分组和批处理策略下整组周期工作的总执行时间，包含输入处理、生成及运行时开销；重叠执行的时间只计一次，不计计划释放时刻之间的空闲间隔，不能把孤立会话耗时直接相加。容量边界处若 `C_N(L_mem)<T`，则仍有 `T-C_N(L_mem)` 的全局执行时间预算。这是显存先于周期计算预算成为限制的条件，不是准入充分条件；还须独立检查各会话的 deadline、恢复窗口与分配约束。单会话的外部阶段耗时只能解释量级，不能替代该条件在目标并发与上下文长度下的测量。
...
令 `w_i` 为该会话逐出字节在本周期内没有 GPU 分配的时长，按字节求平均，从实际释放算到恢复目标预分配，故 `0<=w_i<=T`。平均驻留节省为 `sum(e_i*w_i)/T`；相对于固定全驻留基线的峰值节省是各时刻节省量的最小值，因而 `peak_saving<=average_saving<=E<=B_eff*T`。周期乘带宽给出理想上界，不等于必然新增的可用显存；区间长度和重叠决定实际峰值收益，提前预分配会缩短节省时间。
