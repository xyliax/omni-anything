...
方向性机会：**每一 tick 内的 H2D 搬运带宽与计算时间大量闲置**，是可用来赎回并发容量的两份资源。具体方案待重新设计（原方案路线已于 2026-08-08 废除，历史在 git）。

## 负载的三要素

**tick 的定义。** 双工会话是一条持续的双向音频流，没有离散的「请求—响应」边界；服务侧把它离散化为固定长度 T 的音频切片，一个切片周期即一个 tick。T 由模型的音频接口决定，不是 serving 层的自由参数（谱系分三类，见本节末；本仓真机栈 T = 2 s）。每到 tick 边界，每路活跃会话必须完成一份固定工作：把刚结束的输入切片编码后追加进 context（增量 prefill），并产出下一个输出切片播放所需的 token（decode 配额；真机栈恒 33 token/tick，FINDINGS C2）。这份工作的 deadline 是在本 tick 周期内、留出播放余量完成（真机栈 worker 等待帽 1.6 s < T = 2 s）；过期未产出时播放端只能填充静音或过时内容——用户可闻的正确性事故，而非尾延迟劣化。引擎对 tick 零感知（FINDINGS C2）：tick 结构完全由音频到达节奏从外部塑形，deadline 只存在于 gateway / worker 层——这是「结构性信息缺失」（见下文）的第一例。
...
**事实 1 · 显存先于算力成为瓶颈。** E1 真机（vLLM 0.23 + Qwen2.5-Omni-7B + RTX 3090，tick 2 s，N=8）下 KV pool 饱和有三种失效形态：`full_sequence_must_fit` 的 re-admission 条件加 FCFS `break` 造成 head-of-line blocking 下全部会话死锁、preemption cascade、同步填充下的 admission deadlock（FINDINGS A3）。
...
**事实 2 · 相位从没被当成可调度资源。** 早期模拟器标定：随机相位比相位对齐多花 **4.96 倍** GPU 时间，其中到达偶然聚合成 batch 产生的额外开销占全卡忙时 **64.7%**。
batch size 由会话到达的巧合决定，调度器对此没有控制权（引擎 work-conserving：只要存在就绪工作就立即执行，从不等待）。
真机侧的相位打散 (desync) 守恒律已定：平均 batch 大小 B̄ ≥ N×配额×t_step/T ≈ 2.8，上界 T·BW/(配额×W) ≈ 3.4（3090 + 7B）；相位差 T/N 的连续旋转优于离散槽；同步 tick 下 KV 搬运无收益，时间排他性是 capacity 扩展的必要条件（FINDINGS D3）。
...
- **注入瓶颈**：效率地板上，一 tick 内计算加注入的总需求 = 1 时 **N≈57**（早期模拟器线性外推：baseline 在 N=48 时前台 deadline 与注入延迟两侧均已劣化，48→57 是调度器可争取的空间，超过 57 进入 admission control 领域）。
...
silent failure 让上述三条对帧级 SLO 指标不可见——不是对一切监控不可见，而是失效恰好躲开了这类负载现有的对外指标。
双工通道没有离散的「请求—响应」，静音帧也是合法帧，所以 cadence 类指标（deadline miss、frame delivery、frame latency）度量的是传输而不是语义：崩溃状态下 worker 等待帽 1.6 s < 2 s deadline，每 tick 仍准时返回空帧，miss 恒 0%、frame delivery 100%（FINDINGS B1）；client latency 恒 1 ms（burst 预存了 8 个 tick 的 token buffer，播放端消费的是预生成 token），用户实际经历的是模型在回应 16 s 前说的话——semantic response lag 最多约 16 s（FINDINGS B2）。
失效在引擎内部信号里清晰可见（kv.log 的池占用与 starvation、队列状态），但那些是诊断信号而非 SLO；duplex serving 因此需要独立的 content freshness SLO（按 SRE 意义给内容陈旧度阈值），容量决策不能只看帧级指标与利用率。

## 为什么难：结构性信息缺失与跨层耦合

**信息是结构性缺失，不是没调好。** 这些信息物理上全部可得——tick 周期精确可预测、KV 的下次使用时刻恰是下个 tick、注入大小到达即知、内容按帧冻结、cancellation 信号即时——但在任何引擎的请求模型里都没有字段可放。系统真正要做的是四个决策，每个决策依赖一个缺失字段，而引擎各以一个错误的默认行为代替：

| 缺失字段 | 依赖它的决策 | 引擎的默认行为 | 实测后果 |
|---|---|---|---|
| per-request 的 deadline 与相位 | 相位指派 (phase-offset assignment)：谁占 tick 内哪个子槽 | work-conserving：batch 构成由到达巧合决定 | 随机相位比对齐多花 4.96 倍 GPU 时间（事实 2） |
| KV 的预计下次使用时刻（精确可知：下 tick = 上 tick + T） | H2D 搬运时刻表：release = 内容冻结点，deadline = 计算槽 − τ_lead（prefetch lead time）；静默停泊 (idle-session offload) 是其远期极限情形 | eviction 按 LRU，恢复走 preempt-重算 | LRU 恰好逐掉马上要用的；重算 4k token 花 330 ms prefill，swap 只要约 19 ms（出处见下段） |
| 注入的可延性（delay-tolerant 标记与大小） | 注入安放：落在哪个 tick 的空隙、是否绕开引擎 step | 8-token 唤醒与 8192-token 注入是同一种对象 | 最坏相位 L\* = 6144 即 deadline 超限（事实 3） |
| cancellation 信号（应用层即时可得） | commit / cancel 语义：uncommitted 停 DRAM，作废块立即回收 | 已作废的 KV 和马上要 attend 的 KV 是同一种字节 | 峰值 24.2% 的 resident KV 已作废（事实 4） |
...
显存层 eviction 的恢复路径本身制造或不制造一次注入：重算一个 4k token 会话 = 330 ms prefill（早期标定），等价于额外制造一次注入负载；swap 实测只要 38.1 ms（4k token，pinned H2D 12.33 GB/s，早期标定，原始 bench 数据在 git 历史；此数是标定模型 Qwen3-1.7B 的 112KB/token 口径，7B 的 56KB/token 口径同段约 19 ms）且走 DMA 引擎可与计算重叠——引擎步对拷贝引擎，这对数字既是「用闲置带宽换更大容量」的成本基础，也是重算路径被排除的原因。
有界 sliding window 解开 KV 压力，却使注入无法驻留：几千 token 的工具结果可能还没被说出来就滚出窗外。
KV 方案与注入方案必须联合设计，这是把「链路调度」与「提交语义」做成一体的原因。

## 领域空白

- **Serving 文献**：截至 2026-08，全双工 GPU serving 论文只有 Metronome 一篇（无注入、无作废，负载最简化）；它对 KV 装不下的答案是 sliding window 截断，有损，且与注入需要长期驻留冲突。
- **模型层在承担服务侧缺位的代价**：MoshiRAG 明知插入式注入精度更好却弃用，理由是 "to constrain sequence length"；全部开源双工模型的上下文折合会话时长 ≤20 分钟——领域对 KV 增长的现行答案是「忘掉」。
- **产品层佐证**：Seeduplex 自述克服了「高并发下的延迟尖刺与稳定性问题」，解法不公开；GPT-Live 单会话 ≥1 小时（社区实测），手段不公开。
...
| # | 关系 | 内容 |
|---|---|---|
| 1 | 交叉验证 | 真机（Qwen3-Omni-30B FP8 / 96 G / tick 2 s）独立测得同一瓶颈出现顺序：vanilla 下 memory cliff 先到（计算远未饱和），KV 受限后 deadline 瓶颈 N\*≈209 小于显存外推约 500，与本仓「显存先于算力」同构（数值巧合不构成校准）；引用其数字前先阅读 `docs/metronome.md` 的订正节 |
| 2 | 对照负载 | 它测的是最简化的双工负载：每帧存活量恒定、无注入、无作废——要素 ② 的两半它均未覆盖 |
| 3 | 方案种子 | 刻意错开相位的实验设置升级为相位指派；sliding window 的 state-bounded 世界是有损对照 |
| 4 | 竞争方案 | 它对「KV 装不下」用 W=1024 sliding window（有损，与注入驻留冲突）；本仓方向是同等保真下外扩容量边界（具体方案待重新设计） |
| 5 | 互补可叠加 | 它管 capacity 上限之外（AIMD admission）与有损 cap；本仓方向在瓶颈约束内扩张 capacity |
...
原方案路线已于 2026-08-08 整体废除（实现、设计文档与过程记录移除，历史在 git），架构与算法由项目负责人重新设计。任何新方案仍须回答上面四个实测事实：事实 1 是收益来源（容量先饱和而算力/带宽闲置）；事实 2 要求把相位当成可调度资源；事实 3 要求注入不与前台 tick 抢引擎 step；事实 4 要求 cancellation 语义有落点。

## 明确不做什么

语音合成头与音频 tokenization；过载区 N 超过产能上限（admission control，属 Metronome 的范畴）；多卡 prefill / decode 分离（作参照 baseline 对比，不作贡献）；80 ms 锁步家族（tick 短 6 倍，收益除以 6）。
早期模拟器标定用较小标定模型推演增益，其数字是乐观上界；真机栈（RTX 3090、tick 2 s）与论文主配置（tick 480 ms）不同，数字以 `results/` 下留存 run 的记录为准（重构前的旧证据在 git 历史 `109db82` 及更早）。
