...
这些问题按可依赖关系推进。评估首先联合改变会话规模与上下文条件（Q1），检查 KV 分配是否在计算和输入处理之前限制服务，以建立问题实例；随后在相同参考历史与服务标准下比较不同驻留和恢复方案（Q2、Q3），确定资源节省是否转化为达标承载能力及其延迟代价；机制分析（Q4）进一步固定非目标条件，分别检查主机复制、逐出和不同预取触发的作用；资源模型（Q5）与正确性协议（Q6）解释结果并检验其外推能力；扰动与压力条件（Q7）划定适用边界。对于没有获得服务收益的配置，保留其恢复、计算与等待分解，用于解释边界而非从结果中删除。
...
| 窗口、压缩或摘要等保留策略 | 作为负载参数改变单会话保留状态规模，检验驻留机制在不同保留策略下的可组合性 | 单列语义差异；不承担裁决保留策略之间应用质量的义务 |

不可获得或无法适配的系统应说明原因，并区分重实现、分析参照和实测基线。窗口化等保留策略作为负载参数进入比较，而非作为必败基线；实验不设计使其失败的任务，也不裁决保留策略之间的质量取舍。
...
| 指标 | 定义要求 |
| --- | --- |
| 更新服务延迟 | 起点取实际提交 `a(i,k)`、终点取生成完成；每会话首次提交的对齐等待单列报告；记录输入和结果关联 |
...
**已定义与待定。** 主完成事件定为**本次更新的生成完成**：本次更新的生成结束——无论到达输出配额还是提前终止——即记 `c(i,k)`；用户可见交付建模为其后的常数偏移，网络与客户端因素不属于研究范围（此假设须在论文中显式声明）。当前 worker 实现的映射单列：正常路径生成至输出配额（见[生成工作量差异](#executed-decode-difference)），当前测量路径以跑满配额的配置运行时，"输出配额全部生成"与上述定义重合。延迟基准定为**实际提交 `a(i,k)`**：释放节奏由服务栈入口按指派偏移控制，稳态下提交时刻即偏移网格、`a` 与 `r` 重合；每会话仅首次提交存在一次性对齐等待（至多一个周期），按会话单列报告，不并入稳态分布。该基准必须与"更新落后"指标成对使用——否则入口在过载时顺延提交，会使 `c - a` 达标而输入积压不可见。仍待定：目标值 `D` 与允许违约比例（参数化，按扫描而非单点报告）；观察期限、持续落后与会话失败规则。若各输出架构无法使用同一完成事件，应说明公共要求与各自映射。待定项落实前只报告已定义的量，不写"系统满足 SLO"或"已增加最大并发"。
...
3. 将语义一致与时限达标分开报告；运行未崩溃不能替代其中任何一项。保留策略之间的应用质量取舍属应用侧参数，不在本协议裁决范围内。
...
| `CONTEXT_GROWTH_TOKENS_PER_PERIOD` | 78 | 当前容量分析配置常数，非已确认净保留增长；口径差异（FINDING-E4）见 [Findings](findings.md#diagnostic-appendix-fairness-and-measurement) 执行成本诊断 |
...
两个 first-party worker 当前均使用 `ignore_eos=True`，正常路径运行到不同 cap；模型长度边界和异常仍可能提前停止。Pilarius 即使关闭逐出也建立主机 offload connector；baseline 目前没有同等同步调度配置接口。这些差异影响结果解释的三个环节：生成与保留规则差异改变每周期的状态增长，从而改变容量压力；调度与输出等待差异改变批形状与步时长，从而改变计算成本归因；失败与拒绝规则决定哪些样本进入主指标。修复这些差异后需要重新采集公平对照，不能重新解释旧数据使之看似匹配。

Pilarius 现有接口包括 gateway `--slots`、`--retained-prefix-blocks`、诊断用 `--evict-tail-blocks`、`--prefetch push` 和 `sync_scheduling`。它们不直接等于上文全部消融组；实际路径见 [System](system.md#implementation-boundary)。
...
以下字段因跨系统语义或执行路径差异直接影响公平性与结果解释，保留于本文档；其余纯读数语义字段的逐项解释见 [Analyze Results](agent/tasks/analyze-results.md#field-semantics)。
...
当前 runner 对部分错误、缺失日志和启用机制无事件进行拒绝；这描述运行器规则，不是所有科学问题的通用验收，其跨 runner 差异与覆盖限度由 [contracts registry](agent/contracts.json) 的 CONTRACT-RUN-HEALTH 持有。失败 run 可以支持失效边界分析，但不能记为成功性能点。
