...
**已定义与待定。** 主完成事件定为**本次更新的生成完成**：本次更新的生成结束（无论到达输出配额还是提前终止）即记 `c(i,k)`；用户可见交付建模为其后的常数偏移，网络与客户端因素不属于研究范围（此假设须在论文中显式声明）。当前 worker 实现的映射单列：正常路径生成至输出配额（见[生成工作量差异](#executed-decode-difference)），当前测量路径以生成量达到配额的配置运行时，"输出配额全部生成"与上述定义重合。延迟基准定为**实际提交 `a(i,k)`**：释放节奏由服务栈入口按指派偏移控制，稳态下提交时刻即偏移网格、`a` 与 `r` 重合；每会话仅首次提交存在一次性对齐等待（至多一个周期），按会话单列报告，不并入稳态分布。该基准必须与"更新落后"指标成对使用；否则入口在过载时顺延提交，会使 `c - a` 达标而输入积压不可见。仍待定：目标值 `D` 与允许违约比例（参数化，按扫描而非单点报告）；观察期限、持续落后与会话失败规则。若各输出架构无法使用同一完成事件，应说明公共要求与各自映射。待定项落实前只报告已定义的量，不写"系统满足 SLO"或"已增加最大并发"。
