...
实验改动一律落在本仓库 `experiments/` 或自有 patch，不动 pin 内代码；读写细则的 owner 是 `third_party/AGENTS.md`。
...
竞争对照那一行与本仓库方向直接对立：in-engine windowed KV 用有损截断换有界显存；本仓库的方向是同等保真下外扩容量边界、不截断内容（具体方案待重新设计）。
paper §2 把 swap 分析性排除、断言 resident 是唯一预算兼容选择，其两个假设（全量轮换 baseline、tick 内无空隙）经 FINDINGS E3 证伪——任何走 host↔HBM 搬运路线的方案，对照位置正在这里。
