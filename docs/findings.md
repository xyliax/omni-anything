...
| 机制 | 直观含义 | 实现与语义状态 | 性能结论 | 证据 |
| --- | --- | --- | --- | --- |
| 错开相位（phase staggering） | 分散会话发射时刻 | 已实现；语义已验证 | 测量点结论成立；需正式复跑 | `EVIDENCE-H1-COMPARISON` |
| 取现货交付（take-from-stock delivery） | 返回上一周期库存 | 已实现；语义已验证 | 指标语义已验证 | `EVIDENCE-H2-METRICS` |
| KV 部分释放（park） | 释放可恢复的闲置尾部 | 已实现；语义已验证 | 测量点结论成立；需正式复跑 | `EVIDENCE-H4-RESIDENCY` |
| KV 预取（prefetch） | 提前搬回已释放尾部 | 已实现；源码可审计，原始诊断证据缺失 | 开放：FE 膨胀使净收益尚未闭合 | `EVIDENCE-H7-PREFETCH` |
...
<a id="finding-c2"></a>
### FINDING-C2 — Baseline 每段 33 Token 的来源
...
该结论有两条限定：33/段而交付 25/tick 会产生 **+8/段的库存漂移**（FINDING-F7）；旧 seeded baseline 还会因 `session.max_tokens` 冻结而退化为每段 1 token，不能作为有效证据。当前 baseline 通过 `worker/engine_fix` 在每个 chunk 刷新该字段，conveyor 由 KV 部分释放补丁（park patch）处理并通过 config 约束组合。

<a id="finding-c3"></a>
### FINDING-C3 — 历史运行默认使用异步调度
...
<a id="finding-d1"></a>
### FINDING-D1 — 容量受限时的 Step-Time 不变量
...
<a id="finding-e1"></a>
### FINDING-E1 — Metronome 论文归因取代仓库旧结论
...
<a id="finding-e4"></a>
### FINDING-E4 — DuplexOmni 支持论文 Tick 配置
...
<a id="finding-h2"></a>
### FINDING-H2 — 取现货交付要求重建指标

取现货交付让 `Step` 不再包含 GPU 等待，latency p50 因而退化为 0.15ms、TTFA 错误读数 2ms（真值 ≥ 一个周期）、饥饿完全不可见。重建：`deadline_met` = 本 tick 交付 ≥ tpt（miss = 引擎未跟上），TTFA 如实包含一片流水线滞后（~2005ms），latency 分布永久改走 trace 侧。教训同 F 系列：**评估协议必须随交付语义重建，否则指标全部正常与系统完全失效可以并存**。
证据：`EVIDENCE-H2-METRICS`；口径实现在 conveyor gateway 的 delivery contract。

<a id="finding-h3"></a>
### FINDING-H3 — 主动 KV 部分释放可由 vLLM 既有原语闭合

vLLM 抢占只作用于 RUNNING 请求，闲置 resumable 会话没有释放路径；其 LRU 逐出序对循环负载近似反 Belady，会先逐出最快再次使用的内容。因此，被动等待池满再轮转不可行。

主动 KV 部分释放（park）组合 `free(request)`、`evict_blocks` 与 connector hash-match reload：前两者释放 request grip 并精确销毁尾块，后者按原 hash 恢复。它们都是引擎既有能力，补丁新增的是状态转移。
...
<a id="finding-h4"></a>
### FINDING-H4 — 常量底座自动释放使稳态驻留有界

decode 结束自动释放（auto-park-on-stop）在调度器停止转移处原地执行，无额外 RPC。keep-K 配额把闲置底座固定为 K+2 余量加 1 个未满块：稳态池占用 **0.29**，全驻留假设为 0.860（保留 run 的 tick-window 重算值为 0.292）。同时全驻留会话数为 3，占 79%，接近 slice 时长 × N / 周期给出的物理下界；回载窗口 p50 为 70ms，被逐尾块 CPU 覆盖率 100%，交付未恶化。
...
<a id="finding-h6"></a>
### FINDING-H6 — Warm Start 是带屏障的状态构造
...
屏障收尾**不能执行 KV 部分释放**：镜像发起依赖各请求自身的调度步，已停止会话的剩余 seed 块不会继续发起；低优先级拷贝流还会被连续 seed prefill 占用。观测中只覆盖约 130/256 块，已发起拷贝仅确认 0–27/256。此时释放会销毁无副本块，导致首 tick 重算整段 seed、迟到 1–1.65s，并让库存永久偏移 38 token。

正确收尾只解除自动释放挂起：第一周期全驻留，首次正常 segment stop 再进入部分释放稳态；观测中 8/8 会话覆盖率达到 100%，库存保持 25。饥饿从会话**首次足额交付**起算，与 TTFA 的首 token 语义分离。baseline 公平对照臂已采用同一屏障。

证据：`EVIDENCE-H6-WARM-START`；完整根因链保存在 legacy experiment record。

<a id="finding-h7"></a>
### FINDING-H7 — KV 预取语义闭合但性能仍开放

回载的可复用核心是“让内容存在于 GPU prefix cache”，而不是“回载某个 request 的尾巴”。实现分为四层：registry 保存控制阶段、时序和延迟队列，块级真相按需查 pool；`reload_kv` / `kv_state` 提供指令面；匿名传输完成后按原 hash 注册并转为 LRU 可弃的 cached-free block；真实 request 仍通过 vLLM 原生 resume hash match 认领。失败、迟到或被逐出只会退化为按需回载。

历史诊断中（N=8、seed=4096、K=128、120s，两次复跑），477–478 次预取均完成，拷贝窗口 p50 约 96ms，并消除了大段按需回载；但 tick→prefill p50 仅从 316ms 降至 297ms，远小于理论 70ms，因为 FE 同时从 221ms 膨胀到 270ms。末尾两次短交付也稳定复现，根因仍未闭合。

当前发射策略在 pool 紧张时把 reload 延迟到下一次 KV 部分释放事件重评，chunk 被 demand path 认领后取消延迟项；历史 workload 没有触发预算闸，尚不能说明高压 pacing 已验证。原始预取 run 未保留，registry 将本条标为 `legacy-unreconstructable`；当前源码只能支持语义审查，不能独立重算性能数字。
