...
证据：`20260809_144706_smoke-n8-120s`（61 tick 积压 475 token = 38s 播放量，38/120 ≈ 0.32）；机制在 `experiments/baseline/worker/stream_server.py` 的 `step()`。
...
2026-08-12 追记（conveyor 实现过程中回溯发现的两条限定，baseline 已冻结不修）：①33/段 × 交付 25/tick = **+8/段的库存漂移**（F7 的同一病理）——baseline 数字里凡涉及「交付内容对应哪段音频」的口径须加此限定；②`session.max_tokens` 冻结在构造值（H3 上游 bug ②），**带 seed 的 baseline run 每段固定为 1 token、属无效证据**——已核查两个保留 run（pilot / smoke）均 seed=0，不受影响；conveyor 侧已用 config 校验禁止此组合。
...
证据：`20260810_122257_stagger-golden`、`20260810_183303_stagger-gwtrace`。
...
auto-park-on-stop（在调度器停止转移处原地执行，零延迟零 RPC）+ keep-K 配额（闲置底座固定为 K+2 余量+1 未满块）：稳态池占用 **0.296 vs 全驻留假设 0.860**，同时全驻留会话数 3 个占 79%（≈ slice 时长 × N / 周期的物理下界），回载窗口 p50=70ms、被逐尾块 CPU 覆盖率 100%，交付零恶化。镜像是增量 write-through：稳态 PCIe 上行 ≈ 增长率（~5 块/周期/会话），下行 ≈ 尾巴大小（随 L 线性涨——容量 roofline 的带宽线）。
证据：`20260811_193634_park-onstop`。
