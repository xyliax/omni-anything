...
已有材料包括历史诊断和源码检查，尚无公平比较下的正式性能主结果。下表给出其支持范围；设计见[系统设计](system.md)。
...
| KV 预取 | 已检查的源码包含提交后预取、延后和回退路径 | 基于未来节奏的策略、并发路径验证及可复现净延迟收益 |
...
| <a id="finding-h7"></a>FINDING-H7 | 已检查的源码包含主机状态提前装入、容量不足延后与后续正常复用的路径，目标先分配、完成后发布索引；缓存仍可能在使用前被替换，不足以宣称所有并发回退均已验证 | `EVIDENCE-H7-PREFETCH`（历史性能原始数据 不完整，源码只补语义审查） | 需求超越、重复恢复、引用生命周期、高压竞争与净延迟实验 |
...
下列步时长结果来自 `EVIDENCE-STEP-TIMING-RETAINED`（历史诊断）。统计定义未统一的数值须复算后再用于正式比较。
...
| <a id="finding-e2"></a>FINDING-E2 | 旧汇总报告 decode 步时长 p50 处于 23.5–23.8 ms 区间（正文与结构化记录统计定义尚需复算统一，本稿不选定其一），每增加一个并发序列约增加 0.66 ms；同步路径测得步延迟而异步路径含吞吐归因，工作量未匹配，与权重带宽解释相容只是假说 | HBM 测量、执行窗口定义与因果对照 |
