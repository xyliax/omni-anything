...
def scheduler_trace_environment(trace_path: Path, errors_path: Path) -> dict[str, str]:
...
def apply_scheduler_trace(env: dict[str, str], trace_path: Path, errors_path: Path) -> None:
    """Mutate a child-process environment to enable the scheduler trace."""
    settings = scheduler_trace_environment(trace_path, errors_path)
