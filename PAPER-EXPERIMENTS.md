...
**注意力墙在级联尾部现身（三墙合影）**：SM 相位均值 35.6%（8 路，短 ctx）→50.8%（2 路，25k ctx）——**每会话成本 4.5%→25.4%，随 ctx ~线性 ×6**。外推：显存无限时 N=8 将在 ctx≈12k（t≈320s）撞算力预算。同一实验家族测齐三面墙：摄入（实现债，可修）、显存（本质，conveyor 的靶）、注意力（本质，windowed 砍的就是它/我们靠 TDMA+搬运保上下文）。
