...
- **服务时序发现（`results/figures/E1_service_timeline.png`，脚本 `harness/plot_e1_service_timeline.py`）**：输入端零相位——gateway 每拍一个批量 Step，8 路 P 事件相差 <0.1ms；引擎端**涌现出无控制的粗粒度轮转**：稳态下每会话每 ~15s 获得一个 ~1.9s 的独占服务窗，一口气消化 ~7 个积压 chunk（每 chunk 中位 242ms：encoder+53 tok prefill+decode 配额），随后等 ~13s 轮下一圈。三个推论：① 客户端延迟恒 1ms 的真因——一次 burst 产出 ~175 token，够客户端按 25/拍取 7 拍，缓冲区永不空；② **内容时效性隐患**：一段音频最长积压 ~13s 才被编码，帧准时但语义回应滞后 ~7 拍——cadence 指标完全测不出这层 staleness，双工产品口径需要独立的"内容延迟"指标；③ 对 E3：vanilla 已经在付轮转的全部代价（burst 期间其余 7 路的 KV 白占显存），却没有拿到轮转的好处（无控制、比拍粗 7 倍、无法配合搬运窗口）——TDMA 相位指派是把这个涌现结构变成拍级、可预测、可与传送带预取对齐的版本，"摊平的价值"叙事有了 vanilla 侧的实证锚点。
