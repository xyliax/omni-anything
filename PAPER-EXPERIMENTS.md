...
- **服务时序发现（`results/figures/E1_service_timeline.png`，脚本 `harness/plots/plot_e1_service_timeline.py`）**：输入端零相位——gateway 每拍一个批量 Step，8 路 P 事件相差 <0.1ms；引擎端**涌现出无控制的粗粒度轮转**：每会话获得独占服务窗、一口气消化全部积压 chunk，随后等一整圈。**定量订正（inter-F 分箱分析）**：每 chunk 串行服务时间 **~265ms 且与上下文长度无关（30–240s 全程平坦）**→ N×t_chunk = 8×265ms = 2.12s > 2s 拍 → **ρ≈1.06，摄入管线从 ~30s 起就过载**；积压因此单调增长（chunks/visit：1→2→4→5→7→8，对应内容陈旧度 2s→16s），不是稳态。若显存墙不在 255s 先杀死系统，积压会无界增长——**这是独立于显存墙和注意力墙的第三面墙（摄入串行墙）**。嫌疑定位：t_chunk 与上下文无关 + 引擎死后（255s+）前端仍以 ~240ms/chunk 的节奏继续拉取音频 → 瓶颈**已定罪至代码行**（`vllm/v1/engine/async_llm.py` `handle_inputs`，~L458）：每个 StreamingInput chunk 的 `input_processor.process_inputs`（特征提取+分词+mm哈希+序列化）**同步跑在 AsyncLLM 前端的单条事件循环线程上，无线程池、无对引擎进度的门控（`async for` 贪婪拉取）**；单线程吞吐 1000/265ms≈3.8 chunk/s < 到达 4 chunk/s。burst 轮转的成因：asyncio 任务仅在真挂起时让出，`process_inputs` 同步、非空 `queue.get()` 与缓冲未满的 IPC 发送均不挂起 → 每个会话的摄入任务一口气抽干自己队列才让出 → 涌现每会话独占 burst。引擎死后节奏不变（前端不看引擎死活继续预处理并塞核内缓冲——也解释了 worker 日志 tot_tokens 冻结而 resident_frames 持续上涨）。实测本机 HF processor 单 2s chunk 全程 55ms（特征提取 52ms）——占 265ms 的 ~1/5，其余 ~200ms 为分词/mm哈希/msgpack/zmq/GIL 竞争等逐 chunk 前端常数，M2 可逐段计时。GPU 侧的账：每拍全部 GPU 工作（8×encoder + 424 prefill tok + ~200 decode tok）≈1.0–1.3s < 2s——**GPU 有能力按拍完成，是单线程摄入漏斗喂不进去；实现债而非物理极限**（线程池/批量摄入/ingest 下沉均可修）；另注意本机为 12 用户共享服务器，CPU 侧常数可能被环境放大，且 Metronome N=96 未报告此墙——**已用其 paper 数据裁决（订正先前猜测）**：其 t_wall 随 N 缩短（N=96 ~240s → N=128 ~148s，且早期线性拟合预测 t_sat 误差仅几个百分点）→ 其池填充速率随 N 线性扩展、未被摄入吞吐封顶 → **其主机上前端每 chunk ≤ 2000/128 ≈ 15.6ms，摄入未饱和，其"健康期"是真健康（内容新鲜），先前"其陈旧度或也隐性增长"的猜测撤回**。同一段单线程漏斗代码两家都在跑，差别只在常数：本机 265ms/chunk（HF processor 52ms + ~210ms 主机侧开销：老共享 Xeon 单核慢/GIL 竞争/swap 在用 38GB 疑似加重）vs 现代服务器 CPU 的 ~10-20ms。**正确的论文表述：摄入墙是结构性的（单线程、无背压、无指标暴露），但其绑定点 N_ingest = 拍长/t_chunk 是主机相关的**——任何主机都存在一个 N 使摄入先于 GPU 饱和且无声；我们的 N=8 恰好骑在本机的线上。以本机数字入稿前必须：N=4/6 对照点（ρ<1 验证）+ 干净主机复测 t_chunk。**N=4/6 对照已完成（`e1perreq_n{4,6}_d300_*`，GPU3/300s）：模型全中**——N=4（ρ=0.53）603 次 visit 积压全部恒为 1、零 burst；N=6（ρ=0.80）积压 p50=1/max=2 且末段 60s 不增长（稳定队列）；F−P 排队延迟随 N 单调上升（p50 366ms→591ms→N=8 的 1015ms）。摄入排队诊断因果闭环：ρ<1 不积压，ρ>1（N=8）无界积压。E2 与 vanilla 对比的操作点据此选 N≤6。三个推论：① 客户端延迟恒 1ms 的真因——burst 一次产出 ~8 拍的 token 存货，帧交付永不断供；② **真实的内容时效 SLO（本拍音频本拍处理完）从 ~60s 起对所有会话持续违约且违约量线性增长**——按此口径 N=8 在本硬件上根本不可行（摄入容量 ≈ 2000/265 ≈ 7.5 路），cadence 指标对此完全失明；③ 对 E3：vanilla 已在付轮转的全部代价（burst 期间其余 7 路 KV 白占显存）却没拿到任何好处（无控制、无法配合搬运窗口）——TDMA 是把它变成拍级、确定性、可与传送带预取对齐的版本。

### 2026-08-04：并行摄入补丁实验（e1paringest，N=8/600s/GPU3）——摄入墙证毕即除，显存墙改换形态

**补丁**：`harness/stream_server_paringest.py`——monkeypatch `AsyncLLM._add_streaming_input_request`，唯一改动是每 chunk 的 `process_inputs` 进 `ThreadPoolExecutor(8)`（会话内顺序保持；`mm_processor_cache_gb=0` 消除跨线程共享态）。venv 与克隆零改动。driver `harness/run_e1_paringest.sh`。

**修复验证（一行补丁的因果证明）**：积压全程恒 1（含崩溃期，零轮转）；F−P 从 1015ms → **3ms**（p95 9ms）；时间线图（`results/figures/e1paringest_n8_d600_service_timeline.png`）显示 8 路每拍齐步、紧贴推送线。健康期引擎出现真空闲（run 呼吸式 7↔0，SM 相位均值 35.6%）——GPU 余量直接可见。代价：TTFA 18ms→751ms（t=0 八路首 prefill 同时压 GPU，无害）。

**新故障形态：幸存者通吃级联（`results/figures/E1_cascade_anatomy.png`，脚本 `harness/plots/plot_e1_cascade.py`）**：池打满 kv=1.000 → 抢占一个受害者 → 幸存者以 N_alive×78tok/2s 回填 → 再满再抢。6 次抢占（216.8/247.1/288.4/346.1/432.8/577.1s，间隔 30→41→58→87→144s 精确按 1/N_alive 拉长），终态 2 幸存者（ctx 25.8k≈1.4GB each）。受害者永不复活的棘轮：其复活门票=全序列，饿死后仍被前端喂大（9.4k→24.9k），空闲从未超 ~12%。客户端这次可见 DEGRADING（p99 漂 +861ms）但 miss 仍 0%。

**旧"全员死锁"的机制订正（双 bug 咬合）**：v1 调度器双等待队列——挂起等输入的会话（`WAITING_FOR_STREAMING_REQ`）入 `skipped_waiting`，等显存准入的入 `waiting`，FCFS 下 **skipped 绝对优先**（`scheduler.py:1667`）。串行摄入时代人人有积压 → 人人在 `waiting` 排毒丸（队头受害者）之后 → break 全灭；摄入健康时会话走 skipped 通道绕过毒丸 → 只死受害者。**即 e1perreq 的 run=0/wait=8 全员死锁 = 摄入墙×显存墙的合谋产物**；单独的显存墙表现为级联而非死锁。两种形态都终结于大部分会话死亡+算力闲置，windowed/conveyor 的必要性不变，但论文的 anatomy 叙事要按此分层。

**注意力成本斜率的直接测量（订正：不是"墙现身"——本次运行从未计算 bound）**：修好摄入后每拍的活全程在拍内完成（存活会话每拍足额 25 token、终态 2 路时 SM 忙碌仅 50.8% ≈ 每拍 ~1s/2s）。测到的是斜率：每会话每拍 GPU 忙碌 ~0.09s（ctx~4k）→ ~0.51s（25k），随 ctx 近线性。**外推**（两点线性 + 未计批处理摊薄，仅数量级）：8 路齐长时 8×busy(ctx) 在 ctx≈11–12k 越过 2s 拍预算才成为计算 bound；实际显存墙在 ctx≈8.6k（217s）先动手，当时计算余量 ~3×。计算 bound 的判据以"一拍的活 > 拍长"为准，SM util 只是代理。三墙状态：摄入（实测生效，已修）、显存（实测生效，conveyor 的靶）、注意力（仅测得斜率与外推越界点，未触及）。

**方法论教训（时间线工具）**：kv/smi/perreq 三时钟的偏移必须逐 run 用 warmup 锚点重算（本次 −25.7s vs 上次 −15.6s）——初判"sid7 死于抢占之前"即目测偏移伪象，对齐后 216.988s vs 216.8s 严丝合缝。

### 2026-08-05：拍内执行剖面实测（e1periter，N=8/90s，逐引擎步日志）——两处猜测被数据纠正

**方法**：`_StatLog` 加 `PERITER_LOG`（每引擎步记录 t/run/gen/ptok，不限流），90s 短点（`harness/run_e1_periter.sh`），43 个稳态拍。

**拍内真实剖面（43 拍一致）**：8 路在 **84ms（p50，p95 164ms）内全部进入 batch**（2–4 步）→ **89% 的步是纯 batch=8 decode，21.0ms/步** → 收尾 2–3 步 8→5→0（配额几乎同时用完）→ 忙碌窗合计 811ms/2000ms，其余纯空闲。逐步回放样例在 `e1periter_n8_d90_periter.log`（t=57.55 拍：dt 序列 60/85ms 起步后稳定 21ms×35 步）。
...
### 补注：TDMA 去同步的守恒律与粒度上界（E3/M2 设计参数，2026-08-05）

同步程度是连续旋钮，服从守恒：B̄×每拍步数 = N×配额（264 tok），步数 ≤ T/t_step ≈ 95 ⟹ **B̄ ≥ N×配额×t_step/T ≈ 2.8**。
两端点：完全同步（E1 实测：B̄=8、35 步、busy 35%、峰值常驻=全员、链路无用）↔ 最大去同步（B̄≈3、95 步铺满拍、
busy→100%、任意时刻仅 ~3 路 KV 常驻、链路全程可用）。**去同步 = 用算力 slack 购买常驻缩减**（权重每步重读是代价），
上界 = T·BW/(配额×权重字节) ≈ 3.4（3090+7B）；MoE/高 HBM 卡上界大幅放宽（30B-A3B 步时 4.8-14ms → 10+）→ 进 E6 地形。
**连续旋转优于离散组**：相位差 T/N=250ms 均匀铺开 → 每 250ms 一路进/一路出 → 链路双向各 ~2.1GB/s 恒定
（vs 12.3GB/s 容量，6× 余量；离散 G 组则为 G 倍峰值的脉冲）。E3 三臂坐标：全常驻=B̄8 端点、随机相位=旋钮失控、
TDMA=主动选 B̄≈3 均匀旋转。M2 时刻表参数：相位间隔、τ_lead、B̄ 目标 + 守恒律可行性检查。同步拍下传送带无收益的
根因：忙碌窗内 batch decode 每步读全员 KV → 峰值常驻不降，时间排他性是容量扩展的必要条件（D1 自研 worker 的调度动机）。

### 2026-08-06：级联重复 2/5（e1schtr_n8_d600，全程调度器 trace）——确定性时钟、任意受害者

**复现强度**：六次抢占时刻两次运行逐点吻合 ±0.4s（run1: 217.0/247.1/288.4/346.1/432.8/577.1 vs run2:
216.8/247.2/288.4/346.1/432.9/577.2），间隔序列 30→41→58→87→144s 完全一致——本配置填池远快于会话时长，
级联**时钟完全确定性**（Metronome 亚稳态谱系的确定性端点）。**但受害者身份完全不同**（run1 杀 7,2,6,1,8,3；
run2 杀 3,8,1,5,7,6）——`running.pop()` 的瞬时排列决定，**墙"何时杀人"由池算术决定、"杀谁"任意**——公平性
叙事素材（用户无法预知也无法影响自己是否被牺牲）。步时随 ctx 增长直接测得：23.7ms（早期）→26.2ms（末期 2 路
25k ctx）。**级联泳道图** `results/figures/E1_cascade_lanes.png`（脚本 `harness/plots/plot_e1_cascade_lanes.py`）：
三窗（健康满员瓦片 / 抢占#1 瞬间 sid3 泳道熄灭 / 终局双幸存者宽步长），21ms 步分辨率的完整死亡实录。

### 2026-08-06：种子功能验证通过（smoke#2，SEED=6k/N=8/180s）+ 显存墙第三形态

**种子校准后全部达标**：8 路种子 5867–5989 tok（目标 6k，±2%），3.4–12.3s 完成灌注（~1.2s/路），池起步 ~0.68，
**~125s 撞墙**（vs 自然生长 217s，含灌注开销净省 ~40%；种子越大压缩越多）。重复实验与 ctx 扫描自此走快车道。

**第三种死法（同步准入冻结）**：终态 `kv=1.000 run=0 wait=0 pre=0`——池精确打满于拍边界（种子使 8 路 ctx 完全同步），
无人在跑故无抢占触发，8 路全部滞留 skipped_waiting（statlog 的 wait= 只计 waiting 队列，不含 skipped——测量学备忘），
每拍被促升→准入失败→退回，零 eviction 的全员冻结。至此显存墙三形态齐全：①复合死锁（串行摄入×显存，run=0/wait=8）；
②淘汰级联（并行摄入+ctx 异步耗尽，pre=6，2 幸存者）；③同步准入冻结（并行摄入+ctx 同步，pre=0，全员冻结）——
形态由"池耗尽时刻落在拍内还是拍间"与"摄入是否积压"二维决定，全部终结于多数/全部会话死亡+GPU 闲置。paper 的
anatomy 节按此三分。**静态图规范追加**：时间轴图必须画真实拍边界（P 事件锚定），坐标整数刻度会被误读为节拍
（E1_cascade_lanes 已补虚线；跨时钟锚定法：min(prefill_start−preceding_tick)=+3ms，viewer 与图共用）。

### 补注：撞墙 slack 的硬件不变量与适用边界（E6 建模，2026-08-06）

**撞墙步时不变量**：池满时每 decode 步读 (W+M)/BW ≈ 0.9×显存/BW——模型大小只改变权重/KV 分账，不改总字节。
验证：3090 预测 25.4ms vs 六次抢占实测 24.8–27.2ms。结合 V/BW 五代恒定（24–36ms）与配额随拍长缩放（r×T）：
**busy_wall/T ≈ r×0.9×(V/BW) + 开场占比 ≈ 38–55%（r=语音级）——对模型、拍长、显卡代际三重不变**。
"显存墙撞上时算力恒有约一半空闲"（C1 最强形式）。适用边界：① B > B* ≈ 字节/参数×TFLOPS/(2×BW)
（3090+bf16 ~40–80，H100+bf16 ~200，B200+FP8 ~280）后 MLP 计算 bound 化（缓降非跳变，注意力项恒为带宽 bound）；
...
（收尾偶发第 34 步的候选解释）。**间隔语义细则**：间隔=每步关键路径=max(GPU总工作,CPU工作)。稳态 decode 步
GPU 侧为瓶颈（间隔均匀+物理闭合+双探针一致）→ ≈GPU 步总时（含采样/拷贝杂项）；**拍开场首步在流水线排空后
CPU 串行回到关键路径——48–86ms 的开场步含未知比例 CPU 成分，"encoder+prefill 耗时"降级为上界**（nsys 标定项）；
超前调度可产生不对应 GPU 时间的超短间隔伪影，本系列 trace 实证无此模式，但换版本/配置后应先验间隔分布。
