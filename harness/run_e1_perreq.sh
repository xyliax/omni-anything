...
# Worker = harness/stream_server_perreq.py (instrumented COPY; metronome clone untouched).
...
TAG="e1perreq_n${N}_d${DUR}"

echo "### [$(date +%H:%M:%S)] $TAG fresh worker (GPU $GPU, MML=$MML, ${DUR}s)"
rm -f /tmp/wready_perreq_$$.flag "$OUT/${TAG}_perreq.log" "$OUT/${TAG}_kv.log"
CUDA_VISIBLE_DEVICES=$GPU HF_HUB_OFFLINE=1 METRONOME_ROOT=$MET \
  ...
  setsid ~/vllm023-venv/bin/python -u "$ROOT/harness/stream_server_perreq.py" \
    --model "$MODEL" --port $WPORT \
    --gpu-mem $GPU_MEM --max-model-len $MML --max-num-seqs $MAXSEQS --tpt 25 --max-audio-chunks 64 \
    --window-frames 0 --wait-budget-s 1.6 --ready-file /tmp/wready_perreq_$$.flag \
    > "$OUT/${TAG}_worker.log" 2>&1 &
WPID=$!
for i in $(seq 1 120); do
  [ -f /tmp/wready_perreq_$$.flag ] && break
...
[ -f /tmp/wready_perreq_$$.flag ] || { echo "WORKER TIMEOUT"; kill -9 -$WPID 2>/dev/null; exit 1; }
