...
fig.suptitle("Who is the engine actually serving? — per-session service timeline, vanilla vLLM-realtime, N=8, steady state (t≈200 s)",
             fontsize=10.5, color=INK, x=0.055, ha="left", y=1.00)
for ext in ("png", "pdf"):
    fig.savefig(f"results/figures/E1_service_timeline.{ext}", dpi=200,
                bbox_inches="tight", facecolor=SURF)
print(f"median inter-chunk service {med_dt*1000:.0f} ms; wrote results/figures/E1_service_timeline.png/pdf")
