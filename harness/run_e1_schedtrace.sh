...
CUDA_VISIBLE_DEVICES=$GPU HF_HUB_OFFLINE=1 METRONOME_ROOT=$MET \
  SCHED_TRACE=$OUT/${TAG}_sched.log PYTHONPATH=$ROOT/harness/sched_trace METRONOME_STATLOG=$OUT/${TAG}_kv.log PERREQ_LOG=$OUT/${TAG}_perreq.log \
