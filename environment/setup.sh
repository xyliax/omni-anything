...
PROFILE_DIR="$ROOT/environment/profiles/$PROFILE"
...
usage() {
  cat <<'EOF'
Usage: bash environment/setup.sh [options]
...
while (( $# )); do
  case "$1" in
    --profile) PROFILE="$2"; PROFILE_DIR="$ROOT/environment/profiles/$2"; shift 2 ;;
...
python3 "$ROOT/environment/verify.py" --worker-python "$VENV/bin/python"
