...
本文供作者用 Draw.io 手绘图 1（动机实例时间轴）与图 3（设计总览）使用，是这两张图唯一的画图指南；它合并了此前分散的工作指南、绘图规格与机制理解审计。本文给出两图的画法、事件模型与参数表、共享视觉规范、caption 草稿与审阅流程；少数尚可选择的画法单列在 §2，附带建议。
...
以下几处 §7、§8 给出的是默认画法，作者可以换成另一种。每项写明两种画法各是什么、换法的得失与建议；选定后改本文对应小节，再动图。

| 位置 | 默认画法 | 可选画法 | 得失与建议 |
| --- | --- | --- | --- |
| 图 1(a) 聚合子图 | 一条驻留 KV 总量阶梯对容量虚线 | 按状态堆叠：下层深色为正在计算会话的 KV，上层浅色为 idle 会话的 KV | 堆叠能直接显示"越线时全部 KV 都 idle"，但在对齐相位下深浅随时间整体切换，信息量有限，且增加一层图例。建议保持单条阶梯，把该论点交给 caption |
| 图 1(a) 计算余量 | 不单设条；时间余量由各泳道内约 0.35T 的爆发段与 caption 承载 | 加一条"每周期 GPU 计算时间对周期预算"的低位条，配 100% 预算虚线 | 加条能让"memory 到顶时 compute 仍在山脚"一眼可见，代价是 panel 更高。若加，只能用这一口径；不可用"并发会话数对上限"，它在对齐下打满且虚构每会话独占 GPU。建议加 |
| 图 1(a) 横轴右端 | 止于 3T | 预留约 0.2T，画各泳道下一次 release 的虚线菱形，注 "releases known ahead" | 把"下一次使用时刻可知"这一机会埋进问题面板，但多占一条注释预算。建议不加，机会由 (b) 与 caption 承担 |
| 两图配色 | 单色相：深蓝计算、浅蓝 idle、橙 hatch 在途；会话身份靠泳道标签 | 色相编码会话身份（Okabe-Ito 四色），明度或纹理编码状态；代价与失败处用红色 | 多色相让图 3 中 S1 逐出到 S3 预取的因果由颜色接力讲述，链路轨道四色轮流出现即自明"时分复用"；代价是灰度可读性更依赖明度差，需逐色校验。两图必须同选。建议在图 3 试一版多色相再定 |
| 图 3 pool 子图 | 总量阶梯，无数值刻度 | 按会话堆叠面积 | 堆叠能看出各会话对峰值的贡献，但与数字纪律无冲突的前提是仍不设数值轴。建议保持总量阶梯 |
| 图 3 因果链 | 带圈序号 1–4 各配锚点圆点，因果由 caption 叙述 | 用细弧线箭头把 1→2→3→4 串起来，全图唯一一组箭头 | 箭头让读者不读 caption 就能沿因果走一遍，但四条弧线穿越泳道会与 hatch 和菱形打架。建议先画序号版，排版后若有空间再加箭头 |
...
- **计算余量的口径。** "并发会话数对上限 4"在对齐下会打满（4 个一起 batch），且虚构每会话独占 GPU，不能用。若单独表现时间余量，只能用"每周期 GPU 计算时间对周期预算"的口径，它与相位无关；是否加这一条见 §2。
...
以下为两图当前统一的规范；多色相方案是 §2 列出的可选画法，不与本表混用。
...
- 计算时间余量：每周期 GPU 计算时间对周期预算约三分之一，与相位无关。默认不单设条，该信息由泳道内的爆发段与 caption 承载；加条的做法见 §2。
- 聚合子图按状态堆叠、横轴右端加下一次 release 的虚线菱形，均为 §2 的可选画法，默认不画。
...
- pool 子图按会话堆叠与因果链弧线箭头为 §2 的可选画法，默认不画。
...
- 2026-09-21：合并。将两份英文绘图规格、2026-09-08 机制理解审计与旧图件计划并入本文并删除原件；图号统一为 PAPER.md 编号（图 1 动机、图 3 设计总览，文件名不变）；Metronome 对齐属性记为已核验；视觉规范以单色相方案为当前规范，多色相等尚可选择的画法集中到 §2 并附建议；新增数字纪律、语义红线、不回退旧做法与质量门槛小节；工作流改为 Draw.io 手绘，删除旧生成器相关内容。
