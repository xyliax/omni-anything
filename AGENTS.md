...
**`.context/` 是工作语境层：进行中状态、用户-agent 讨论、设计探索与外部原料；不构成项目事实。**

归属测试：删掉该文件，项目正确性或可理解性是否受损？受损 → `docs/`；只是「重新收集要花时间」→ `.context/`。

| 维度 | `docs/`（事实层） | `.context/` |
| --- | --- | --- |
| 内容 | 本项目结论、问题、方案、实验、纪律 | 工作状态与讨论（worklog）、设想（ideas）、外部整理（references / papers）、表达草稿（slides） |
| 过时 | 过时是 bug | `worklog/` 过时也是 bug（会话收尾更新，完结即删）；其余允许滞后，快照打日期即可 |
...
提升通道（单向）：`worklog/` 完结、`ideas/` 被采纳 → 结论写入 `docs/`（worklog 文件随之删除，git 历史留底）；digest 中项目依赖的结论上移，原文留 `.context/papers/`。
...
| `.context/worklog/` | 进行中工作的状态与用户-agent 讨论记录；一题一文件 | 会话收尾时更新；完结即删，结论走提升通道 |
...
- 进行中状态与未决讨论看 `.context/worklog/`；数字一律以 `docs/findings.md`、`results/`、`docs/experiment-log.md` 为准。**不要**根据 `.context/` 其余子目录或第三方 pin 反推进展、主线或数字。
