...
Outputs
  ...
  -> results/e0_dma_interference/runs/<UTC-run-id>/{measurements.csv,summary.json}
...
def default_output_dir() -> Path:
    """Choose a new evidence directory; the benchmark never overwrites a prior run."""
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S.%fZ")
    root = Path(__file__).resolve().parents[2]
    return root / "results" / "e0_dma_interference" / "runs" / stamp
...
def main():
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="new run directory; defaults to a UTC-stamped directory under results/",
    )
    args = ap.parse_args()

    # Keep argument discovery usable on a fresh checkout. The CUDA stack is
    # intentionally loaded only after argparse has handled --help.
    global torch, AutoConfig, AutoModelForCausalLM, StaticCache
    try:
        import torch
        ...
        ap.error(
            ...
            "install the project environment with environment/setup.sh"
        )
    ...
    try:
        ...
        ap.error(f"refusing to reuse existing run directory: {output_dir}")
    measurements_path = output_dir / "measurements.csv"
    summary_path = output_dir / "summary.json"

    dev = args.device
    torch.cuda.set_device(dev)
    cfg = AutoConfig.from_pretrained(args.model, revision=args.model_revision)
    ...
    with measurements_path.open("x", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)
    mid = [x["kappa"] for x in rows if 0 < x["rate"] <= 0.75]
    summary = {"model": args.model, "model_revision": args.model_revision,
