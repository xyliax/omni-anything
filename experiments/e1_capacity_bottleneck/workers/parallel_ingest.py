...
Run through experiments/e1_capacity_bottleneck/run.sh with the environment built by environment/setup.sh:
...
class StreamingEngine:
    ...
    async def _run_session(self, sid: int, st: Session):
        ...
        while not st.stop:
            ...
            async def gen():
                # 0) WARM-START: one-shot prefill of approximately seed_tokens.
