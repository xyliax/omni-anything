...
新双工推理引擎（项目负责人设计，增量实现）。三个机制增量均已实现并有权威证据（结论在 `docs/findings.md` H 系列，稳定证据入口为 `results/conveyor/runs/`）：**错开相位**、**取现货交付**、**park 驻留管理**（镜像/park/回载全链路）。每次运行起一个全新 worker，证据先落入带 ID 的不可变目录；版本库只保留最新且有效的一份。
