"""KV partial-release primitive, injected into the spawned EngineCore process.

The conveyor runner prepends this directory to ``PYTHONPATH`` (ahead of
tracekit's collector dir — Python imports only the FIRST ``sitecustomize`` on
``sys.path``, so this module chain-loads the scheduler-trace collector at the
end). Nothing is patched unless ``OMNI_PARK_PATCH`` is set. Note the worker
process itself also imports this file (PYTHONPATH applies to every python
child); attaching the method there is harmless — only the EngineCore process
ever receives the utility call.
...
above remains for fixed-tail experiments. A second wrapper around ``_update_request_as_session`` does
three per-chunk jobs: reset the eager-store cursor (upstream bug: it
accumulates duplicates on streaming re-entry), refresh ``session.max_tokens``
(upstream bug: the stop check reads a value frozen at construction — the
seed's max_tokens=1 would pin every segment to 1 token), and zero
``num_computed_tokens`` for parked sessions so resume takes the match path.
...
``<epoch> L req=<id> cpu_tok=<n> gpu_tok=<m>`` (reload admitted,
WAITING_FOR_REMOTE_KVS) and ``<epoch> R req=<id>`` (reload complete).
tracekit parses all four (parse_park) and renders them as PARK instants,
KV mirror instants and paired KV-reload slices; the runner treats a
park-enabled run whose log holds zero park lines as a run issue.
"""
...
if os.environ.get("OMNI_PARK_PATCH"):
    try:
        ...
        def _park_request(scheduler, request, nblocks=0, keep_blocks=None,
            ...
            _log_park(f"{time.time():.6f} req={request.request_id} {fields}\n")
            return result
        ...
        # parks everyone in one burst and releases this gate.
        ...
        def _traced_remote_done(self, request):
            _log_park(f"{time.time():.6f} R req={request.request_id}\n")
        ...
        def _traced_store_specs(self, scheduler_output):
            ...
            for req_id, state in self._reqs_to_store.items():
                ...
                if delta > 0:
                    _log_park(f"{now:.6f} S req={req_id} blocks={delta}\n")
            return result

        SimpleCPUOffloadScheduler._prepare_eager_store_specs = _traced_store_specs
    except Exception as error:
        _fail("initialization", error)
        # A requested mechanism must not silently vanish: a "successful" run
        # that never parked would masquerade as evidence. 78 = EX_CONFIG,
        # matching the tracekit collector's convention.
        os._exit(78)


# Chain-load the scheduler-trace collector (this module shadows it on
# sys.path whenever the park dir is prepended).
if os.environ.get("OMNI_SCHEDULER_TRACE"):
    try:
        import importlib.util

        _root = os.path.dirname(  # engine_patch -> worker -> conveyor -> experiments -> repo
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        )
        _trace_path = os.path.join(
            _root, "tracekit", "collectors", "vllm_scheduler_trace", "sitecustomize.py"
        )
