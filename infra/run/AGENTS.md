...
dirty source 的诊断运行在启动任何进程前保存 `source.patch`，包含相对 HEAD 的 tracked diff 与新增 first-party source/docs/tests；不递归打包 `results/`。patch 捕获失败则不启动实验。该 artifact 和 manifest 一起参与终态 hash，可在基准 commit 上重建当时源码；它不把 dirty run 升格为正式性能证据。

`RunPlan.before_client` / `after_client` 是有界的顺序控制命令；前者在客户端启动前完成，后者仅在客户端确实退出后、worker 清理前执行。它们参与 provenance，失败不能得到成功终态。GPU capture 通过该接口覆盖完整业务，导出不在业务中途执行。
