...
快速上手（详情见 [`environment/README.md`](environment/README.md) 与各实验目录的 AGENTS.md）：

```bash
bash environment/setup.sh --profile cuda13_vllm023 --download-models   # 建 venv + 补丁 + 编译两个 gateway + 拉锁定模型
python3 environment/verify.py --worker-python .venv-vllm023/bin/python # 校验环境
